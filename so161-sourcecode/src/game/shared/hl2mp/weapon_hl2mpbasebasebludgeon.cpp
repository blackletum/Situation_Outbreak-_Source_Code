...
void CBaseHL2MPBludgeonWeapon::ItemPostFrame( void )
{
	...
	else if ( (pOwner->m_nButtons & IN_FIREMODE) && (m_flNextSecondaryAttack <= gpGlobals->curtime) )
	{
		SecondaryAttack();
	}
	else 
...
void CBaseHL2MPBludgeonWeapon::PrimaryAttack()
{
	...
	// S:O - Kick the player's views, just like we do for all the melee weapons' secondary attack.
	AddViewKick();
