...
BEGIN_NETWORK_TABLE( CBaseGrenade, DT_BaseGrenade )
	...
	// S:O - Used to determine whether or not the grenade in question is an incendiary grenade.
	SendPropBool (SENDINFO(Burnnade) ),
	...
	// S:O - Used to determine whether or not the grenade in question is an incendiary grenade.
	RecvPropBool ( RECVINFO(Burnnade) ),
...
void CBaseGrenade::Explode( trace_t *pTrace, int bitsDamageType )
{
	...
	// S:O - Incendiary Grenade Support
	if( IsIncendiary() )
	{
		float effectRadius = 128, duration = 10;
		...
		for ( CEntitySphereQuery sphere( GetAbsOrigin(), effectRadius ); ( pEntity = sphere.GetCurrentEntity() ) != NULL; sphere.NextEntity() )
		{
			if(pEntity)
			{
				...
				if( FStrEq(classname, "prop_physics_multiplayer") || FStrEq(classname, "prop_static") || FStrEq(classname, "prop_physics_override") || FStrEq(classname, "prop_physics"))
				{
					CEntityFlame *pFlame = CEntityFlame::Create( pEntity, false );
					if ( pFlame != NULL )
					{
						pFlame->SetParent( this );
						pFlame->SetLifetime( 10.0 );
						pFlame->SetSize( 20 );
					}
			}
			...
			{ // make sure they're in LOS - MASK_NPC_BRUSHONLY means that someone standing
			  // between me & the grenade won't stop it affecting me
				trace_t tr;
				...
				if ( tr.m_pEnt == pEntity || tr.fraction == 1.0f )
					if(pEntity->GetTeamNumber() == 3 && friendlyfire.GetInt() == 1)
					{
						//return;
					}
					else if(pEntity->GetTeamNumber() == 3 && friendlyfire.GetInt() == 0)
					{

					}
					else if(pEntity->GetTeamNumber() != TEAM_SPECTATOR || pEntity->GetTeamNumber() != 0)
					{
						pEntity->MyCombatCharacterPointer()->Ignite(duration,false);
					}
			}
		}
	...
	// S:O - Only remove the grenade if it isn't an incendiary so that we can start our fires and smoke properly.
	if( !IsIncendiary() )
	{
	...
	// S:O - More Incendiary Grenade Stuff
	SetTouch( NULL );
	SetSolid( SOLID_NONE );
	...
	// Light the area around the explosion on fire!
	...
	if ( pFlame != NULL )
	{
		...
		pFlame->SetLifetime( 10.0 );
		pFlame->SetSize( 100 );
		pFlame->SetLocalOrigin( vec3_origin );
		pFlame->SetMoveType( MOVETYPE_NONE );
	}
