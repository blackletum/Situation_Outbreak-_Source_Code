...
CBaseCombatWeapon::CBaseCombatWeapon()
{
	...
	// S:O - Clip System for Weapons (enabled/on by default)
...
#if !defined( CLIENT_DLL )
	// S:O - We want all weapons to blink when they spawn.
	//if( IsX360() )
	//{
...
void CBaseCombatWeapon::Drop( const Vector &vecVelocity )
{
	...
	if( FClassnameIs( this, "weapon_zombie" ) || FClassnameIs( this, "weapon_charple" ) || FClassnameIs( this, "weapon_corpse" ) || FClassnameIs( this, "weapon_stalker" ) || FClassnameIs( this, "weapon_brassknuckles" ) )
	{
		return;
	}
...
{ 
	...
	if ( GetOwner()->IsPlayer() ) // AI Patch Addition.
		SetModel( GetWorldModel() );
#endif
...
#if !defined(CLIENT_DLL)
			if( !IsX360() || !ClassMatches("weapon_sniper") )
#endif
			{
				bFired = true;
			}
...
#ifndef CLIENT_DLL
	// Add a bit of a delay between "reload" shouts to prevent spam and excessive noise.
	if ( (GetTeamNumber() == 3) && (gpGlobals->curtime >= m_flNextReloadShout) )
	{
		...
		{
			EmitSound( "Reload.Female" );
		}

		// For now, let's try a delay of ten seconds.
		m_flNextReloadShout = gpGlobals->curtime + 10.0f;
	}
#endif
