...
CGameMovement::CGameMovement( void )
{
	...
	GAMEMOVEMENT_JUMP_DELAY_REAPER = 3000.0f;
...
#if defined(HL2_DLL)
	// S:O - Are we a living reaper? If so, manage our special jumping ability
	if ( player->IsStalker() )
	{
		// If we're ducking, charge up our jump special ability
		if ( player->GetFlags() & FL_DUCKING )
		{
			if ( player->m_Local.m_flSuperJumpTime < 5000.0f )
			{
				player->m_Local.m_flSuperJumpTime += frame_msec;
				if ( player->m_Local.m_flSuperJumpTime > 5000.0f )
				{
					player->m_Local.m_flSuperJumpTime = 5000.0f;
				}
			}
		}
		// If we're not ducking, uncharge our jump special ability
		else
		{
			if ( player->m_Local.m_flSuperJumpTime > 0.0f )
			{
				float frame_msec_double = 2.0f * frame_msec;
				player->m_Local.m_flSuperJumpTime -= frame_msec_double;
				if ( player->m_Local.m_flSuperJumpTime < 0.0f )
				{
					player->m_Local.m_flSuperJumpTime = 0.0f;
				}
			}
		}
	}
...
void CGameMovement::Accelerate( Vector& wishdir, float wishspeed, float accel )
{
	...
	currentspeed = sqrt( DotProduct(mv->m_vecVelocity, mv->m_vecVelocity) ); //AI Patch Addition.
...
bool CGameMovement::CheckJumpButton( void )
{
	...
	if ( g_bMovementOptimizations )
	{
		...
		//flMul = 160.0f;	// approx. 21 units. //Modification Movement.Removal.

		// Modification Movement Addition: so jump height is determined by settings in gamemovement.h jump height/time.
		// S:O - Are we a living stalker? If so, manage our special jumping ability
		if ( (player->IsStalker()) && (player->GetFlags() & FL_DUCKING) && (player->m_Local.m_flSuperJumpTime > 0.0f) )
		{
			float calcdistjumpup = (float)GAMEMOVEMENT_JUMP_HEIGHT_REAPER / (float)(5000.0f / player->m_Local.m_flSuperJumpTime);
			flMul = sqrt(2 * sv_gravity.GetFloat() * calcdistjumpup);
		}
		else
		{
	...
	if ( (player->IsStalker()) && (player->GetFlags() & FL_DUCKING) && (player->m_Local.m_flSuperJumpTime > 0.0f) )
	{
		...
		float calcdistjumpforward = (float)GAMEMOVEMENT_JUMP_DISTANCE_REAPER / (float)(5000.0f / player->m_Local.m_flSuperJumpTime);
		VectorAdd( (vecForward*calcdistjumpforward), mv->m_vecVelocity, mv->m_vecVelocity );
		player->m_Local.m_flSuperJumpTime = 0.0f;
	}
#endif
	...
	if ( gpGlobals->maxClients == 1 )
	{
		player->m_Local.m_flJumpTime = GAMEMOVEMENT_JUMP_TIME_REAPER;
