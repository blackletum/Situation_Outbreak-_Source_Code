...
void CBaseEntity::FireBullets( const FireBulletsInfo_t &info )
{
	...
	// bullet's final direction can be changed by passing through a portal
		...
		if ( tr.DidHit() )
		{
			...
			if ( psurf != NULL )
			{
				HandleBulletPenetration(info, tr, vecDir, vecEnd, &traceFilter, info.m_iDamage, psurf->game.material );
			}
		}
...
bool CBaseEntity::ShouldDrawUnderwaterBulletBubbles()
{
	...
	CBaseEntity *pPlayer = UTIL_GetNearestVisiblePlayer(this); // AI Patch Addition.
...
// S:O - Bullet Penetration
void CBaseEntity::HandleBulletPenetration(const FireBulletsInfo_t &info, const trace_t &tr, const Vector &vecDir, const Vector &vecEnd, ITraceFilter *pTraceFilter, const int weapondamage, unsigned short material )
{
	// Move through the glass until we're at the other side
	float MAX_PENETRATION = 64.0f;
	Vector	testPos = tr.endpos + ( vecDir * MAX_PENETRATION );
	...
	// Steve: here's something to make it a bit more realistic
	float flDistanceTravelled = ( penetrationTrace.endpos - tr.endpos ).Length();
	float flDamageLossPerInch; // new damage = old damage - ( loss per inch * distance )

	if( tr.m_pEnt && ( tr.m_pEnt->IsPlayer() || tr.m_pEnt->IsNPC() ) ) // player bullet penetration
	{
		if( weapondamage > 25 || ( tr.hitgroup == HITGROUP_LEFTARM || tr.hitgroup == HITGROUP_RIGHTARM
				|| tr.hitgroup == HITGROUP_LEFTLEG || tr.hitgroup == HITGROUP_RIGHTLEG ) || !tr.m_pEnt->IsAlive()
			) // if bullet damage is over 25, hit in the limbs, or the player died the bullet will go through
		{
			flDamageLossPerInch = 0.2f;
		}
		else
			return;
	}
	else
	{
		switch( material )
		{
			...
			case CHAR_TEX_GRATE:
			case CHAR_TEX_CLIP:
				flDamageLossPerInch = 2.0f;
				break;

			case CHAR_TEX_DIRT:
			case CHAR_TEX_SAND:
				flDamageLossPerInch = 1.25f;
				break;

			case CHAR_TEX_PLASTIC:
			case CHAR_TEX_WOOD:
				flDamageLossPerInch = 0.5f;
				break;

			case CHAR_TEX_COMPUTER:
			case CHAR_TEX_VENT:
				flDamageLossPerInch = 0.7f;

			default:
				flDamageLossPerInch = 0.8f;
		}
	}

	float flNewDamage = weapondamage - ( flDamageLossPerInch - flDistanceTravelled );
	...
	FireBulletsInfo_t behindWall;
	behindWall.m_iShots = 1;
	behindWall.m_vecSrc = penetrationTrace.endpos;
	behindWall.m_vecDirShooting = vecDir;
	behindWall.m_vecSpread = vec3_origin;
	behindWall.m_flDistance = info.m_flDistance*( 1.0f - tr.fraction );
	behindWall.m_iAmmoType = info.m_iAmmoType;
	behindWall.m_iTracerFreq = 1; // penetrated rounds always have tracer
	behindWall.m_iDamage = flNewDamage;
	behindWall.m_pAttacker = info.m_pAttacker ? info.m_pAttacker : this;
	behindWall.m_nFlags = info.m_nFlags;
	
	CBaseEntity::FireBullets( behindWall );
}
