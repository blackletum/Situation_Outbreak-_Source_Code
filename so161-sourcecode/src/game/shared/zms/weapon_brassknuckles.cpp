...
// Based on the classic crowbar.
// But dare I say BETTER?
// These things are GANGSTA BITCHES!!!

#include "cbase.h"
#include "weapon_brassknuckles.h"
...
IMPLEMENT_NETWORKCLASS_ALIASED( WeaponBrassKnuckles, DT_WeaponBrassKnuckles )

BEGIN_NETWORK_TABLE( CWeaponBrassKnuckles, DT_WeaponBrassKnuckles )
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA( CWeaponBrassKnuckles )
END_PREDICTION_DATA()

LINK_ENTITY_TO_CLASS( weapon_brassknuckles, CWeaponBrassKnuckles );
PRECACHE_WEAPON_REGISTER( weapon_brassknuckles );

//#ifndef CLIENT_DLL

acttable_t	CWeaponBrassKnuckles::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE(CWeaponBrassKnuckles);
...
// Constructor
//-----------------------------------------------------------------------------
CWeaponBrassKnuckles::CWeaponBrassKnuckles( void )
{
}

// S:O - Override default ItemPostFrame method
void CWeaponBrassKnuckles::ItemPostFrame( void )
{
...
//-----------------------------------------------------------------------------
float CWeaponBrassKnuckles::GetDamageForActivity( Activity hitActivity )
{
	return 37.5f;
}
...
//-----------------------------------------------------------------------------
void CWeaponBrassKnuckles::AddViewKickSecondary( void )
{
...
// Purpose: Add in a view kick for this weapon
//-----------------------------------------------------------------------------
void CWeaponBrassKnuckles::AddViewKick( void )
{
	CBasePlayer *pPlayer  = ToBasePlayer( GetOwner() );
...
//-----------------------------------------------------------------------------
ConVar sk_brassknuckles_lead_time( "sk_brassknuckles_lead_time", "0.9" );

int CWeaponBrassKnuckles::WeaponMeleeAttack1Condition( float flDot, float flDist )
{
	// Attempt to lead the target (needed because citizens can't hit manhacks with the crowbar!)
	...
	float dt = sk_brassknuckles_lead_time.GetFloat();
...
//-----------------------------------------------------------------------------
void CWeaponBrassKnuckles::HandleAnimEventMeleeHit( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
...
// Animation event
//-----------------------------------------------------------------------------
void CWeaponBrassKnuckles::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBrassKnuckles::Drop( const Vector &vecVelocity )
{
	// S:O - This weapon cannot be dropped.
	// Search the mod's solution for "weapon_brassknuckles" for details as to why.
}
...
void CWeaponBrassKnuckles::Swing()
{
	trace_t traceHit;
...
void CWeaponBrassKnuckles::Hit(trace_t &traceHit)
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
void CWeaponBrassKnuckles::SecondaryAttack()
{
#ifndef CLIENT_DLL
