...
IMPLEMENT_NETWORKCLASS_ALIASED( WeaponCleaver, DT_WeaponCleaver )

BEGIN_NETWORK_TABLE( CWeaponCleaver, DT_WeaponCleaver )
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA( CWeaponCleaver )
END_PREDICTION_DATA()
...
// Constructor
//-----------------------------------------------------------------------------
CWeaponCleaver::CWeaponCleaver( void )
{
}

// S:O - Override default ItemPostFrame method
void CWeaponCleaver::ItemPostFrame( void )
{
...
//-----------------------------------------------------------------------------
float CWeaponCleaver::GetDamageForActivity( Activity hitActivity )
{
	return 50.0f;
}
...
//-----------------------------------------------------------------------------
void CWeaponCleaver::AddViewKickSecondary( void )
{
...
// Purpose: Add in a view kick for this weapon
//-----------------------------------------------------------------------------
void CWeaponCleaver::AddViewKick( void )
{
	CBasePlayer *pPlayer  = ToBasePlayer( GetOwner() );
...
//-----------------------------------------------------------------------------
ConVar sk_cleaver_lead_time( "sk_cleaver_lead_time", "0.9" );

int CWeaponCleaver::WeaponMeleeAttack1Condition( float flDot, float flDist )
{
	// Attempt to lead the target (needed because citizens can't hit manhacks with the crowbar!)
	...
	float dt = sk_cleaver_lead_time.GetFloat();
...
//-----------------------------------------------------------------------------
void CWeaponCleaver::HandleAnimEventMeleeHit( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
...
// Animation event
//-----------------------------------------------------------------------------
void CWeaponCleaver::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponCleaver::Drop( const Vector &vecVelocity )
{
#ifndef CLIENT_DLL
...
void CWeaponCleaver::Hit(trace_t &traceHit)
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
void CWeaponCleaver::SecondaryAttack()
{
#ifndef CLIENT_DLL
