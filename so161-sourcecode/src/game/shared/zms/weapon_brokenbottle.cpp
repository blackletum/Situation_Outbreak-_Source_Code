...
IMPLEMENT_NETWORKCLASS_ALIASED( WeaponBrokenBottle, DT_WeaponBrokenBottle )

BEGIN_NETWORK_TABLE( CWeaponBrokenBottle, DT_WeaponBrokenBottle )
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA( CWeaponBrokenBottle )
END_PREDICTION_DATA()

LINK_ENTITY_TO_CLASS( weapon_brokenbottle, CWeaponBrokenBottle );
PRECACHE_WEAPON_REGISTER( weapon_brokenbottle );

//#ifndef CLIENT_DLL

acttable_t	CWeaponBrokenBottle::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE(CWeaponBrokenBottle);
...
// Constructor
//-----------------------------------------------------------------------------
CWeaponBrokenBottle::CWeaponBrokenBottle( void )
{
}

// S:O - Override default ItemPostFrame method
void CWeaponBrokenBottle::ItemPostFrame( void )
{
...
//-----------------------------------------------------------------------------
float CWeaponBrokenBottle::GetDamageForActivity( Activity hitActivity )
{
	return 25.0f;
}
...
//-----------------------------------------------------------------------------
void CWeaponBrokenBottle::AddViewKickSecondary( void )
{
...
// Purpose: Add in a view kick for this weapon
//-----------------------------------------------------------------------------
void CWeaponBrokenBottle::AddViewKick( void )
{
	CBasePlayer *pPlayer  = ToBasePlayer( GetOwner() );
...
//-----------------------------------------------------------------------------
ConVar sk_brokenbottle_lead_time( "sk_brokenbottle_lead_time", "0.9" );

int CWeaponBrokenBottle::WeaponMeleeAttack1Condition( float flDot, float flDist )
{
	// Attempt to lead the target (needed because citizens can't hit manhacks with the crowbar!)
	...
	float dt = sk_brokenbottle_lead_time.GetFloat();
...
//-----------------------------------------------------------------------------
void CWeaponBrokenBottle::HandleAnimEventMeleeHit( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
...
// Animation event
//-----------------------------------------------------------------------------
void CWeaponBrokenBottle::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponBrokenBottle::Drop( const Vector &vecVelocity )
{
#ifndef CLIENT_DLL
...
void CWeaponBrokenBottle::Swing()
{
	trace_t traceHit;
...
void CWeaponBrokenBottle::Hit(trace_t &traceHit)
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
void CWeaponBrokenBottle::SecondaryAttack()
{
#ifndef CLIENT_DLL
