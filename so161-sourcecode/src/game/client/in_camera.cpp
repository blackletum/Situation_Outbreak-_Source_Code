...
// S:O - We don't need cheats to use thirdperson in this mod.
...
void CInput::CAM_Think( void )
{
	...
	C_BasePlayer* pPlayerTemp = C_BasePlayer::GetLocalPlayer();
	C_HL2MP_Player *pLocalPlayer = dynamic_cast<C_HL2MP_Player*>( pPlayerTemp );

	if ( pLocalPlayer )
	{
		// S:O - If we're coming out of firstperson from thirdperson, reactivate our thirdperson mode.
		if ( /*(pLocalPlayer->IsInAVehicle()) && */(pLocalPlayer->IsAlive()) && (pLocalPlayer->GetTeamNumber() != 1) && (pLocalPlayer->GetWaterLevel() < 3) && (!CAM_IsThirdPerson()) && (pLocalPlayer->m_bSwitchedToThirdperson) )
		{
			CAM_ToThirdPerson();
			pLocalPlayer->m_bSwitchedToThirdperson = false;
		}
		// S:O -
		// If we're dead, get out of thirdperson and revert back to firstperson, otherwise things get all funky.
		// Thirdperson while underwater is fucked up too, so prevent us from being able to use it in that instance as well.
		else if ( ( /*(pLocalPlayer->IsInAVehicle()) || */(!pLocalPlayer->IsAlive()) || (pLocalPlayer->GetTeamNumber() == 1) || (pLocalPlayer->GetWaterLevel() == 3) ) && (CAM_IsThirdPerson()) )
		{
			CAM_ToFirstPerson();
			pLocalPlayer->m_bSwitchedToThirdperson = true;
		}
	}
...
void CInput::CAM_ToFirstPersonSpecial(void)
{
	m_fCameraInThirdPerson = false;
	cam_command.SetValue( 0 );
}
...
static ConCommand thirdperson( "thirdperson", ::CAM_ToThirdPerson, "Switch to thirdperson camera."/*, FCVAR_CHEAT*/ );	// S:O - We don't need cheats to use thirdperson in this mod.
...
void CInput::Init_Camera( void )
{
	...
	// S:O - By default, we're in firstperson mode when we spawn, not thirdperson mode.
	m_fCameraInThirdPerson = false;
}