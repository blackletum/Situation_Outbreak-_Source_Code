...
IMPLEMENT_CLIENTCLASS_DT(C_HL2MP_Player, DT_HL2MP_Player, CHL2MP_Player)
	...
	RecvPropInt( RECVINFO( m_bIsInfected ) ),
...
// S:O - New and Modified ConVar Declaratons
static ConVar cl_playermodel( "cl_playermodel", "none", FCVAR_USERINFO | FCVAR_ARCHIVE | FCVAR_SERVER_CAN_EXECUTE, "obsolete");
static ConVar cl_defaultweapon( "cl_defaultweapon", "weapon_9mm", FCVAR_USERINFO | FCVAR_ARCHIVE, "Default Spawn Weapon");
...
void C_HL2MP_Player::ClientThink( void )
{
	...
	// S:O - Make survivors blink if a player is a zombie so that they can see their prey better.
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		CBasePlayer *pTarget = UTIL_PlayerByIndex( i );

		if ( pTarget && this->IsLocalPlayer() )
		{
			if ( (pTarget->GetTeamNumber() == TEAM_SURVIVORS || pTarget->GetTeamNumber() == TEAM_MILITARY) && (this->GetTeamNumber() == TEAM_ZOMBIES) && (pTarget != this) )
				pTarget->AddEffects( EF_ITEM_BLINK );
			else
				pTarget->RemoveEffects( EF_ITEM_BLINK );
		}
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
// S:O - Modified for full thirdperson support.
void C_HL2MP_Player::AddEntity( void )
{
	BaseClass::AddEntity();

	if( (this != C_BasePlayer::GetLocalPlayer()) || (this == C_BasePlayer::GetLocalPlayer() && input->CAM_IsThirdPerson()) )
	{
		if ( IsEffectActive( EF_DIMLIGHT ) )
		{
			...
			if ( (this == C_BasePlayer::GetLocalPlayer()) && (input->CAM_IsThirdPerson()) )
			{
				GetViewModel()->GetAttachment( 1, vecOrigin, eyeAngles );
			}
	...
	else /*if ( !(IsEffectActive( EF_DIMLIGHT )) && (m_pFlashlightBeam) )*/
	{
		ReleaseFlashlight();
	}
}

// S:O - Declare this here.
// It's a little hack I whipped up to get the function below to call what it needs to correctly.
void FormatViewModelAttachmentFlashlight( Vector &vOrigin, bool bInverse );

//-----------------------------------------------------------------------------
// Purpose: Creates, destroys, and updates the flashlight effect as needed.
//-----------------------------------------------------------------------------
// S:O - Modified flashlight function that attaches our flashlight to our weapon model.
// TODO: It looks for a muzzleflash attachment, so make sure we add one to each weapon before release - including melee weapons, as strange as that may seem.
void C_HL2MP_Player::UpdateFlashlight()
{
	if ( (this == C_BasePlayer::GetLocalPlayer()) && (IsEffectActive( EF_DIMLIGHT )) && (!input->CAM_IsThirdPerson()) )
	{
		...
		::FormatViewModelAttachmentFlashlight( vecLightOrigin, true );
		AngleVectors( angLightDir, &vecForward, &vecRight, &vecUp );
		m_pHL2MPFlashLightEffect->UpdateLight( vecLightOrigin, vecForward, vecRight, vecUp, FLASHLIGHT_DISTANCE );
	}
...
ConVar so_cl_draw_own_shadows( "so_cl_draw_own_shadows", "1", FCVAR_ARCHIVE );
ShadowType_t C_HL2MP_Player::ShadowCastType( void ) 
{
	if ( !IsVisible() && so_cl_draw_own_shadows.GetBool() && IsLocalPlayer()  )
...
void C_HL2MP_Player::ItemPreFrame( void )
{
	...
	// S:O - In this mod, we're allowed to use our zoom function to help shoot at a target accurately.
...
// S:O - Modified for firstperson ragdolls.
...
// S:O - Leveling System
// NOTE: MAKE SURE THESE MATCH UP WITH THE DEFINES IN hl2mp_player.cpp AT ALL TIMES!!!
...
// S:O - Leveling System
int C_HL2MP_Player::GetXPNeededForNextLevel()
{
	int experienceForNextLevel = -1;

	if ( GetLevel() == 1 )
	{
		experienceForNextLevel = XP_FOR_LEVEL_2;
	}
	else if ( GetLevel() == 2 )
	{
		experienceForNextLevel = XP_FOR_LEVEL_3;
	}
	else if ( GetLevel() == 3 )
	{
		experienceForNextLevel = XP_FOR_LEVEL_4;
	}
	else if ( GetLevel() == 4 )
	{
		experienceForNextLevel = XP_FOR_LEVEL_5;
	}
...
// S:O - Leveling System
int C_HL2MP_Player::GetXPNeededForCurrentLevel()
{
	int experienceForCurrentLevel = 0;

	if ( GetLevel() == 2 )
	{
		experienceForCurrentLevel = XP_FOR_LEVEL_2;
	}
	else if ( GetLevel() == 3 )
	{
		experienceForCurrentLevel = XP_FOR_LEVEL_3;
	}
	else if ( GetLevel() == 4 )
	{
		experienceForCurrentLevel = XP_FOR_LEVEL_4;
	}
	else if ( GetLevel() == 5 )
	{
		experienceForCurrentLevel = XP_FOR_LEVEL_5;
	}
