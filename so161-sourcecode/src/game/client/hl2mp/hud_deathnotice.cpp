...
#include "zms/clientmode_sonormal.h"
...
void CHudDeathNotice::Paint()
{
	...
	int yStart = GetClientModeSONormal()->GetDeathMessageStartHeight();
...
void CHudDeathNotice::FireGameEvent( IGameEvent * event )
{
	...
	bool npckill = event->GetBool( "npckill" );	// S:O - See if we were killed by an NPC or not

	// S:O - If we were killed by an NPC, display that NPC's name.
	// If we were not, check some other stuff before giving in.
	if ( npckill )
	{
		killer_name = killedwith;
	}
	else
	{
		if ( !killer_name )
			killer_name = "";
		else if ( Q_strcmp( killer_name, "unconnected" ) == 0 )
			killer_name = "Spooky Thing of Doom";
		else if ( Q_strcmp( killer_name, "ERRORNAME" ) == 0 )
			killer_name = "Spooky Thing of Doom";
	}
	...
	deathMsg.iSuicide = ( killer == victim || Q_strcmp( killedwith, "worldspawn" ) == 0 );
	...
	else
	{
		...
		// S:O - Not sure if we really need this anymore...
