...
void CHL2MPClientScoreBoardDialog::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	{
		m_pPlayerCountLabel_DM->SetFgColor( COLOR_UNASSIGNED );
		m_pScoreHeader_DM->SetFgColor( COLOR_UNASSIGNED );
		m_pDeathsHeader_DM->SetFgColor( COLOR_UNASSIGNED );
		m_pPingHeader_DM->SetFgColor( COLOR_UNASSIGNED );
		m_pPingLabel_DM->SetFgColor( COLOR_UNASSIGNED );
	}
	...
	{
		m_pPlayerCountLabel_R->SetFgColor( COLOR_HUMANS );
		m_pScoreHeader_R->SetFgColor( COLOR_HUMANS );
		m_pScoreLabel_R->SetFgColor( COLOR_HUMANS );
		m_pDeathsHeader_R->SetFgColor( COLOR_HUMANS );
		m_pPingHeader_R->SetFgColor( COLOR_HUMANS );
		m_pPingLabel_R->SetFgColor( COLOR_HUMANS );
	}
	...
	if ( m_pPlayerCountLabel_C && m_pScoreHeader_C && m_pScoreLabel_C && m_pDeathsHeader_C && m_pPingHeader_C && m_pPingLabel_C )
	{
		...
		{
			m_pPlayerCountLabel_C->SetFgColor( COLOR_MILITARY );
			m_pScoreHeader_C->SetFgColor( COLOR_MILITARY );
			m_pScoreLabel_C->SetFgColor( COLOR_MILITARY );
			m_pDeathsHeader_C->SetFgColor( COLOR_MILITARY );
			m_pPingHeader_C->SetFgColor( COLOR_MILITARY );
			m_pPingLabel_C->SetFgColor( COLOR_MILITARY );
		}
		else
		{
			m_pPlayerCountLabel_C->SetFgColor( COLOR_ZOMBIES );
			m_pScoreHeader_C->SetFgColor( COLOR_ZOMBIES );
			m_pScoreLabel_C->SetFgColor( COLOR_ZOMBIES );
			m_pDeathsHeader_C->SetFgColor( COLOR_ZOMBIES );
			m_pPingHeader_C->SetFgColor( COLOR_ZOMBIES );
			m_pPingLabel_C->SetFgColor( COLOR_ZOMBIES );
		}
	}
...
void CHL2MPClientScoreBoardDialog::Reset()
{
	...
	InitPlayerList( m_pPlayerListR, TEAM_SURVIVORS );

	if ( FStrEq( so_gamemode.GetString(), "Extermination" ) )
		InitPlayerList( m_pPlayerListC, TEAM_MILITARY );
	else
		InitPlayerList( m_pPlayerListC, TEAM_ZOMBIES );

	// S:O - Do this again to ensure we get the right team colors for Extermination
	m_pScoreHeader_C = (Label*)FindChildByName( "C_ScoreHeader" );
	m_pDeathsHeader_C = (Label*)FindChildByName( "C_DeathsHeader" );
	m_pPingHeader_C = (Label*)FindChildByName( "C_PingHeader" );
...
void CHL2MPClientScoreBoardDialog::UpdateTeamInfo()
{
	...
	int endTeam = TEAM_UNASSIGNED;

	if ( HL2MPRules()->IsTeamplay() )
	{
		if ( FStrEq( so_gamemode.GetString(), "Extermination" ) )
		{
			startTeam = TEAM_SURVIVORS;
			endTeam = TEAM_MILITARY;
		}
		else
		{
			startTeam = TEAM_ZOMBIES;
			endTeam = TEAM_SURVIVORS;
		}
	}
		...
		if ( team )
		{
			...
			switch ( teamIndex ) 
			{
				case TEAM_SURVIVORS:
					...
					break;
				case TEAM_MILITARY:
					teamName = g_pVGuiLocalize->Find( "#HL2MP_ScoreBoard_Military" );
...
void CHL2MPClientScoreBoardDialog::UpdatePlayerList()
{
	...
	for( int playerIndex = 1 ; playerIndex <= MAX_PLAYERS; playerIndex++ )
	{
		if( g_PR->IsConnected( playerIndex ) )
		{
			...
			{
				if ( g_PR->GetTeam( playerIndex ) == TEAM_SURVIVORS )
				{
					pPlayerList = m_pPlayerListR;
				}
				else if ( g_PR->GetTeam( playerIndex ) == TEAM_ZOMBIES )
				{
					// S:O - Fixes a bug that displayed undead as military (lol)
					if ( FStrEq( so_gamemode.GetString(), "Extermination" ) )
						pPlayerList = m_pPlayerListDM;
					else
						pPlayerList = m_pPlayerListC;
				}
				else if ( g_PR->GetTeam( playerIndex ) == TEAM_MILITARY )
				{
					pPlayerList = m_pPlayerListC;
				}
			}
