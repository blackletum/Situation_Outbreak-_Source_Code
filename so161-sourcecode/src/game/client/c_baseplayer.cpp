...
BEGIN_RECV_TABLE_NOBASE( CPlayerLocalData, DT_Local )
	...
	// S:O - The amount of time a reaper will have to wait before they can super-jump again
	RecvPropFloat	(RECVINFO(m_flSuperJumpTime)),
...
// -------------------------------------------------------------------------------- //
	IMPLEMENT_CLIENTCLASS_DT(C_BasePlayer, DT_BasePlayer, CBasePlayer)
		...
		RecvPropInt		(RECVINFO( version) ),	// TODO: Again, is this still used/needed? If not, let's get rid of it.
...
BEGIN_PREDICTION_DATA_NO_BASE( CPlayerLocalData )
	...
	// S:O - The amount of time a reaper will have to wait before they can super-jump again
	DEFINE_PRED_FIELD( m_flSuperJumpTime, FIELD_FLOAT, FTYPEDESC_INSENDTABLE ),
...
// THE FOLLOWING FLASHLIGHT CODE IS NOT USED BY THE MOD
// SEE C_HL2MP_PLAYER.CPP FOR MORE THE CORRECT UPDATEFLASHLIGHT() FUNCTION
...
void C_BasePlayer::UpdateFlashlight()
{
	...
	{
		// We've decided to turn on our flashlight. Yay. ^_^
		if (!m_pFlashlight)
		{
	...
	{
		// We've turned off our flashlight. Time to delete it! >=D
		delete m_pFlashlight;
		m_pFlashlight = NULL;
	}
}
...
bool C_BasePlayer::IsCharple( void )
{
	// If we're not a zombie, we're not a valid reaper (hax0rs!)
	if ( GetTeamNumber() != 2 )
		return false;

	// If we're not alive, we're not really a reaper anymore
	if ( !IsAlive() )
		return false;

	// If we've made it this far and are using a reaper model, we must be a valid reaper
...
bool C_BasePlayer::IsCorpse( void )
{
	// If we're not a zombie, we're not a valid reaper (hax0rs!)
	if ( GetTeamNumber() != 2 )
		return false;
...
bool C_BasePlayer::IsStalker( void )
{
	// If we're not a zombie, we're not a valid reaper (hax0rs!)
	if ( GetTeamNumber() != 2 )
		return false;
