...
public:
	...
	void HandleBulletPenetration( const FireBulletsInfo_t &info, const trace_t &tr, const Vector &vecDir, const Vector &vecEnd, ITraceFilter *pTraceFilter, const int weapondamage, unsigned short material );
...
public:
	...
	// S:O - Need these for client to ID zombies and headcrabs for hud_overview.
	virtual bool					IsZombie( void ) { return false; }
	virtual bool					IsHeadcrab( void ) { return false; }
...
public:
	...
	// S:O - Add m_iMaxHealth, now that it's networked and all.
	int								m_iMaxHealth;
