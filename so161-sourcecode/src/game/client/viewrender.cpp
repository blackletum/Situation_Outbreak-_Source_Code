...
ConVar shader_nightvision( "shader_nightvision", "0", FCVAR_CLIENTDLL );
extern ConVar has_round_ended;
extern ConVar client_timescale;
...
void CViewRender::RenderView( const CViewSetup &view, int nClearFlags, int whatToDraw )
{
	...
	else
	{
		...
		// S:O - Draw our post processing effects.
		// IMPORTANT NOTE: This must be done AFTER DrawViewModels is called and BEFORE Push2DView, or else shaders will effect everything, including 2D views and such.
...
static void NightVision_f ( void )
{
	...
	CBasePlayer *pPlayer = dynamic_cast<CBasePlayer*>( local );

	if ( bDisplayed )
	{
		if ( (pPlayer->GetTeamNumber() == 3 || pPlayer->GetTeamNumber() == 4) && (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80) && (shader_nightvision.GetInt() == 1) && (has_round_ended.GetInt() == 0) && (pPlayer->IsAlive()) )
		{
			const Vector *vOrigin = &pPlayer->GetAbsOrigin();
			CLocalPlayerFilter filter;
			pPlayer->EmitSound(filter, 0, "Nightvision.Off", vOrigin);
		}
	...
	{
		if ( (pPlayer->GetTeamNumber() == 3 || pPlayer->GetTeamNumber() == 4) && (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80) && (shader_nightvision.GetInt() == 0) && (has_round_ended.GetInt() == 0) && (pPlayer->IsAlive()) )
		{
			const Vector *vOrigin = &pPlayer->GetAbsOrigin();
			CLocalPlayerFilter filter;
			pPlayer->EmitSound(filter, 0, "Nightvision.On", vOrigin);
		}

		shader_nightvision.SetValue( 1 );
	}

	bDisplayed = !bDisplayed;
}

// S:O - Nightvision ConCommand Declaration
static ConCommand r_nightvision( "r_nightvision", NightVision_f );

// S:O - Post Processing (Shaders)
// This is where it all happens, boys. SOAK IT IN!!!
void CViewRender::PerformPostProcessEffects( int x, int y, int width, int height )
{
	...
	if ( pPlayer->GetTeamNumber() == 2 )
	{
		if ( g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80 ) 
		{
			if ( !pPlayer->IsAlive() )
			{
				IMaterial *pBlackandWhiteShader = materials->FindMaterial( "post_blackandwhite", TEXTURE_GROUP_CLIENT_EFFECTS, true );
				if ( pBlackandWhiteShader )
					DrawScreenEffectMaterial( pBlackandWhiteShader, x, y, width, height );
			...
			{
				IMaterial *pZombievisionShader = materials->FindMaterial( "post_zombievision", TEXTURE_GROUP_CLIENT_EFFECTS, true );
				if ( pZombievisionShader )
					DrawScreenEffectMaterial( pZombievisionShader, x, y, width, height );

				IMaterial *pZombievisionOverlay = materials->FindMaterial( "HUD/zombievision", TEXTURE_GROUP_OTHER, true );
				if ( pZombievisionOverlay )
					view->SetScreenOverlayMaterial( pZombievisionOverlay );

				cvar->FindVar("mat_fullbright")->SetValue(0);
			}
			else if ( (shader_nightvision.GetInt() == 1) && (has_round_ended.GetInt() == 0) && (pPlayer->IsAlive()) )
			{
				IMaterial *pZombievisionShader = materials->FindMaterial( "post_zombievision", TEXTURE_GROUP_CLIENT_EFFECTS, true );
				if ( pZombievisionShader )
					DrawScreenEffectMaterial( pZombievisionShader, x, y, width, height );
	...
	else if ( (pPlayer->GetTeamNumber() == 3 || pPlayer->GetTeamNumber() == 4) )
	{
		if ( g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80 ) 
		{
			if ( !pPlayer->IsAlive() )
			...
			{
				IMaterial *pNightvisionShader = materials->FindMaterial( "post_nightvision", TEXTURE_GROUP_CLIENT_EFFECTS, true );
				if ( pNightvisionShader )
					DrawScreenEffectMaterial( pNightvisionShader, x, y, width, height );

				IMaterial *pNightvisionOverlay = materials->FindMaterial( "HUD/nightvision", TEXTURE_GROUP_OTHER, true );
				if ( pNightvisionOverlay )
					view->SetScreenOverlayMaterial( pNightvisionOverlay );

				cvar->FindVar("mat_fullbright")->SetValue(1);
			}
			else if ( FStrEq(client_timescale.GetString(), "0.3") )
			{
				IMaterial *pSlowmotionShader = materials->FindMaterial( "post_slowmotion", TEXTURE_GROUP_CLIENT_EFFECTS, true );
				if ( pSlowmotionShader )
					DrawScreenEffectMaterial( pSlowmotionShader, x, y, width, height );
