...
ConVar	client_timescale( "client_timescale", "1.0", FCVAR_HIDDEN, "Sets the scale of time for client-side physics (ragdolls)" );
...
bool PhysicsDLLInit( CreateInterfaceFn physicsFactory )
{
	...
	if ( IsX360() )
	{
		...
		client_timescale.SetValue(0.9f);
	}
...
#endif
		physenv->Simulate( frametime * client_timescale.GetFloat() );
...
{
	return physenv->GetSimulationTime() + gpGlobals->frametime * client_timescale.GetFloat();
}
