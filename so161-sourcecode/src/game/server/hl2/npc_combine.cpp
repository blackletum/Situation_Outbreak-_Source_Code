...
{
	s_iszShotgunClassname = FindPooledString( "weapon_doublebarrel" );
	BaseClass::Activate();
}
...
void CNPC_Combine::PostNPCInit()
{
	if( IsElite() )
	{
		...
		// an AR2. 
		if( !GetActiveWeapon() || !FClassnameIs( GetActiveWeapon(), "weapon_m4" ) )
		{
...
void CNPC_Combine::StartTask( const Task_t *pTask )
{
	...
	case TASK_COMBINE_SIGNAL_BEST_SOUND:
		...
		{
			CBasePlayer *pPlayer = UTIL_GetNearestVisiblePlayer(this); // AI Patch Addition.
		...
		case COMBINE_AE_GREN_TOSS:
			{
				...
				if( m_NPCState == NPC_STATE_SCRIPT )
				{
					...
					Fraggrenade_Create( vecStart, vec3_angle, vecThrow, vecSpin, this, COMBINE_GRENADE_TIMER, true, false );
				}
				else
				{
					// Use the Velocity that AI gave us.
					Fraggrenade_Create( vecStart, vec3_angle, m_vecTossVelocity, vecSpin, this, COMBINE_GRENADE_TIMER, true, false );
					m_iNumGrenades--;
				}
		...
		case COMBINE_AE_GREN_DROP:
			{
				...
				Fraggrenade_Create( vecStart, vec3_angle, m_vecTossVelocity, vec3_origin, this, COMBINE_GRENADE_TIMER, true, false );
				m_iNumGrenades--;
			}
			handledEvent = true;
			break;
...
WeaponProficiency_t CNPC_Combine::CalcWeaponProficiency( CBaseCombatWeapon *pWeapon )
{
	...
	else if( FClassnameIs( pWeapon, "weapon_doublebarrel" )	|| FClassnameIs( pWeapon, "weapon_pumpshotty" ) || FClassnameIs( pWeapon, "weapon_autoshotty" ) )
	{
