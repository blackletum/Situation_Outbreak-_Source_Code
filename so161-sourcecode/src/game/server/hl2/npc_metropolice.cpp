		...
		for ( int i = 0; i < nAIs; i++ )
		{
			...
			if ( ppAIs[i]->Classify() == CLASS_METROPOLICE && FClassnameIs( ppAIs[i], "npc_metropolice" ) )
			{
				...
				if ( pNPC->HasSpawnFlags( SF_METROPOLICE_ALLOWED_TO_RESPOND ) )
				{
					...
					CBasePlayer *pPlayer = UTIL_GetNearestVisiblePlayer(this); // AI Patch Addition.
					if ( pPlayer && FVisible(pNPC) && pNPC->FVisible( pPlayer ) &&  // AI Patch Addition.
...
{
	CBasePlayer *pPlayer = UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
	if ( !GetEnemy() && HasCondition( COND_IN_PVS ) && pPlayer && !pPlayer->IsAlive() ) // AI Patch Addition.
	{
...
void CNPC_MetroPolice::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	{
		CHL2_Player *pPlayer = dynamic_cast<CHL2_Player *>(UTIL_GetNearestPlayer(pHitEntity->GetAbsOrigin())); // AI Patch Addition.
