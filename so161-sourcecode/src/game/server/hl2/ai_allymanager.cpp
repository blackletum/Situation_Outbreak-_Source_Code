...
/*	if ( !AI_IsSinglePlayer() ) // AI Patch Addition.
	{ // AI Patch Addition.
		// @TODO (toml 10-22-04): no MP support right now // AI Patch Addition.
	...
	*/
	...
	for ( int i = 0; i < nAIs; i++ )
	{
		if ( ppAIs[i]->IsAlive() && ppAIs[i]->IsPlayerAlly() )
		{
			...
			// They only count if I can use them.
			if( ppAIs[i]->IRelationType( UTIL_GetNearestPlayer(ppAIs[i]->GetAbsOrigin()) ) != D_LI ) // AI Patch Addition.
				continue;

			// Skip distant NPCs
			Vector vNearestPlayerPos = UTIL_GetNearestPlayer(ppAIs[i]->GetAbsOrigin())->GetAbsOrigin(); // AI Patch Addition.
			if ( !ppAIs[i]->IsInPlayerSquad() && 
				!UTIL_FindClientInPVS( ppAIs[i]->edict() ) && 
				( ( ppAIs[i]->GetAbsOrigin() - vNearestPlayerPos ).LengthSqr() > 150*12 || // AI Patch Addition.
				  fabsf( ppAIs[i]->GetAbsOrigin().z - vNearestPlayerPos.z ) > 192 ) ) // AI Patch Addition.
				continue;
