...
{
	PrecacheModel("models/Military/soldier.mdl");
...
void CProtoSniper::Spawn( void )
{
	...
	/// HACK:
	SetModel( "models/Military/soldier.mdl" );
...
void CProtoSniper::Event_Killed( const CTakeDamageInfo &info )
{
	if( !(m_spawnflags & SF_SNIPER_NOCORPSE) )
	{
		...
		pGib = CreateRagGib( "models/Military/soldier.mdl", GetLocalOrigin(), GetLocalAngles(), (vecForward * flForce) + Vector(0, 0, 600), flFadeTime, bShouldIgnite );
...
int CProtoSniper::SelectSchedule ( void )
{
	...
	if( !UTIL_GetNearestVisiblePlayer(this)->IsAlive() && m_bKilledPlayer ) // AI Patch Addition.
	{
...
void CProtoSniper::StartTask( const Task_t *pTask )
{
	...
	case TASK_SNIPER_PLAYER_DEAD:
		{
			m_hSweepTarget = UTIL_GetNearestVisiblePlayer(this); // AI Patch Addition.
			SetWait( 4.0f );
...
CBaseEntity *CProtoSniper::PickDeadPlayerTarget()
{
	...
	CBaseEntity *pTarget = UTIL_GetNearestVisiblePlayer(this); // AI Patch Addition.
	CBaseEntity *pEntities[ iSearchSize ];

	int iNumEntities = UTIL_EntitiesInSphere( pEntities, iSearchSize, pTarget->GetAbsOrigin(), 180.0f, 0 ); // AI Patch Addition.
