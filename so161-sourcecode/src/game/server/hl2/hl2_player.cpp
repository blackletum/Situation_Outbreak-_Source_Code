...
// S:O - Made some changes to the normal and sprinting speeds, as sprint isn't used in this mod.
// No need to note the default values, really. Who cares. lawl
#ifdef HL2MP
	#define	HL2_WALK_SPEED 100
	#define	HL2_NORM_SPEED 190
	#define	HL2_SPRINT_SPEED 190
#else
...
void CC_Player_Drop( void )
{
	...
	// S:O - Fixes a nasty server crashing bug (forgot to check for NULL, that's all)
	if( pPlayerItem )
	{
		// S:O - Default weapons cannot be dropped!
		if( FClassnameIs( pPlayerItem, "weapon_zombie" ) || FClassnameIs( pPlayerItem, "weapon_charple" ) || FClassnameIs( pPlayerItem, "weapon_corpse" ) || FClassnameIs( pPlayerItem, "weapon_stalker" ) || FClassnameIs( pPlayerItem, "weapon_brassknuckles" ) )
		{
			return;
		}
...
static ConCommand drop("drop", CC_Player_Drop, "Drop current item."); 
...
//#ifndef _XBOX 
	...
	// S:O - Do not allow players to zoom.
	/*if ( IsSuitEquipped() )
	{
...
//					Warning( "In train mode with no train!\n" );
	...
	// S:O - This mod allows us to shoot while zooming.
...
{
	// S:O - Just in case I missed a call of this, I'm going to comment it out.
	// I really don't know why Valve decided to include this in multiplayer mods...
...
// S:O - Public function used to determine whether or not a player is walking at any given time
bool CHL2_Player::IsPlayerWalking( void )
{
	return m_fIsWalking;
}
...
bool CHL2_Player::Weapon_CanSwitchTo( CBaseCombatWeapon *pWeapon )
{
	...
	// S:O - Always allow the player to switch to weapons they don't have ammo for so that he or she can drop that weapon if they'd like
...
void CLogicPlayerProxy::Activate( void )
{
	...
	if ( m_hPlayer == NULL )
	{
		m_hPlayer = UTIL_GetLocalPlayer(); // AI Patch Addition.
	}
}
