...
extern ConVar so_reaper_health;
extern ConVar so_reaper_health_dd;
...
void CFastZombie::Spawn( void )
{
	...
	m_iHealth			= so_reaper_health.GetInt();
	m_flFieldOfView		= 0.2;
	...
	{
		m_iHealth = so_reaper_health_dd.GetInt();
		m_iMaxHealth = so_reaper_health_dd.GetInt();
	}
	else
	{
		m_iHealth = so_reaper_health.GetInt();
		m_iMaxHealth = so_reaper_health.GetInt();
	}
}
...
{
	// S:O - Players AND NPCs so that we don't get stuck together and act stupid
