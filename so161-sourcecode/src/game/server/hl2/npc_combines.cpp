...
ConVar combine_guard_spawn_health( "combine_guard_spawn_health", "0" );

extern ConVar sk_plr_dmg_buckshot;	
extern ConVar sk_plr_num_shotgun_pellets;

//Whether or not the combine should spawn health on death
ConVar	combine_spawn_health( "combine_spawn_health", "0" );

// S:O - Additional ConVars
extern ConVar so_soldier_health;

LINK_ENTITY_TO_CLASS( npc_combine_s, CNPC_CombineS );

// S:O - Connect to new client version
IMPLEMENT_SERVERCLASS_ST(CNPC_CombineS, DT_NPC_CombineS)
END_SEND_TABLE()
...
{
	// Add this military unit to our global military unit list!
	gEntList.m_MilitaryList.AddToTail(this);
}

CNPC_CombineS::~CNPC_CombineS()
{
	// Remove this military unit from our global military unit list!
	// NOTE: It's dead.
	gEntList.m_MilitaryList.FindAndRemove(this);
}
...
void CNPC_CombineS::Spawn( void )
{
	...
	// S:O - Should never be Elite for now, but whatever, we'll leave this here just in case
	if( IsElite() )
	{
		// Stronger, tougher.
		SetHealth( so_soldier_health.GetInt() );
		SetMaxHealth( so_soldier_health.GetInt() );
		SetKickDamage( 10.0 );
	}
	else
	{
...
void CNPC_CombineS::Precache()
{
	...
	// S:O - Always false...for now
	m_fIsElite = false;

	if( !GetModelName() )
	{
		SetModelName( MAKE_STRING( "models/Military/soldier.mdl" ) );

		int randcombineskin = RandomInt(1,3);
		switch ( randcombineskin )
		{
			case 1:
				SetSkin(0);
				break;
	...
	UTIL_PrecacheOther( "weapon_healthkit" );
...
void CNPC_CombineS::Event_Killed( const CTakeDamageInfo &info )
{
	...
	{
		CNPC_Citizen *pSurvivorNPC = dynamic_cast<CNPC_Citizen*>(pEnt);
		CNPC_FloorTurret *pTurretNPC = dynamic_cast<CNPC_FloorTurret*>(pEnt);

		if ( pSurvivorNPC || pTurretNPC )
		{
	...
	if ( pPlayer != NULL )
	{
		...
		{
			DropItem( "weapon_healthkit", WorldSpaceCenter()+RandomVector(-4,4), RandomAngle(0,360) );
...
bool CNPC_CombineS::IsHeavyDamage( const CTakeDamageInfo &info )
{
	...
	if ( info.GetAmmoType() == GetAmmoDef()->Index("AR2") || info.GetAmmoType() == GetAmmoDef()->Index("AK47") )
		return true;

	// 357 rounds are heavy damage
	if ( info.GetAmmoType() == GetAmmoDef()->Index("Deagle") || info.GetAmmoType() == GetAmmoDef()->Index("Sniper") )
		return true;
