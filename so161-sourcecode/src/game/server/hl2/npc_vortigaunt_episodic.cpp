...
int CNPC_Vortigaunt::RangeAttack1Conditions( float flDot, float flDist )
{
	...
	if ( IsAntlionWorker( GetEnemy() ) )
	{
		...
		CBasePlayer *pPlayer = UTIL_GetNearestPlayer(GetEnemy()->GetAbsOrigin()); // AI Patch Addition.
...
bool CNPC_Vortigaunt::HealGestureHasLOS( void )
{
	...
	CBaseEntity *pTargetEnt = UTIL_GetNearestVisiblePlayer(this); // AI Patch Addition.
	if ( pTargetEnt == NULL )
		return false;
