...
extern ConVar so_creeper_health;
extern ConVar so_creeper_health_dd;
...
void CZombie::Spawn( void )
{
	...
	m_iHealth = so_creeper_health.GetInt();
	m_flFieldOfView		= 0.2;
	...
	{
		m_iHealth = so_creeper_health_dd.GetInt();
		m_iMaxHealth = so_creeper_health_dd.GetInt();
	}
	else
	{
		m_iHealth = so_creeper_health.GetInt();
		m_iMaxHealth = so_creeper_health.GetInt();
	}
}
...
void CZombie::PrescheduleThink( void )
{
  	if( gpGlobals->curtime > m_flNextMoanSound )
  	{
 		...
 		{
			// S:O - Less moaning
			m_flNextMoanSound = gpGlobals->curtime + random->RandomFloat( 1.0, 12.0 );
...
void CZombie::SetZombieModel( void )
{
	...
	int randclassiczombie = random->RandomInt(1,17);
	switch( randclassiczombie )
	{
			case 1:
