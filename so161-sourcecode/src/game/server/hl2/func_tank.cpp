...
#else
	m_iSmallAmmoType	= GetAmmoDef()->Index("9mm");
	m_iMediumAmmoType	= GetAmmoDef()->Index("MP5K");
...
void CFuncTank::Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CBaseEntity *pAttacker, bool bIgnoreSpread )
{
	...
	if( pAttacker && pAttacker->IsPlayer() )
	{
		if ( IsX360() )
		{
			UTIL_GetNearestPlayer(GetAbsOrigin())->RumbleEffect( RUMBLE_AR2, 0, RUMBLE_FLAG_RESTART | RUMBLE_FLAG_RANDOM_AMPLITUDE ); // AI Patch Addition.
		}
		else
		{
...
void CFuncTankCombineCannon::MakeTracer( const Vector &vecTracerSrc, const trace_t &tr, int iTracerType )
{
	...
	{
		Vector vecPlayer = UTIL_GetNearestVisiblePlayer(this)->EyePosition(); // AI Patch Addition.
