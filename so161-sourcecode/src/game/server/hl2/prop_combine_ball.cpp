...
void CPropCombineBall::WhizSoundThink()
{
	...
	{
		CBasePlayer *pPlayer = UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition. Slightly different location due to OLD/NEW sdk code differences. Unsure if needed.
		...
		if ( pHitEntity->PassesDamageFilter( info ) )
		{
			...
			else
			{
				...
				if ( (m_nState != STATE_HOLDING) )
				{
					...
					{
						gamestats->Event_WeaponHit( pPlayer, false, "weapon_m4", info );
					}
