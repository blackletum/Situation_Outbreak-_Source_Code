...
void CNPC_PlayerCompanion::Precache()
{
	...
	gm_iszShotgunClassname = AllocPooledString( "weapon_doublebarrel" );
...
void CNPC_PlayerCompanion::GatherConditions()
{
	...
	if ( pPlayer && pPlayer->IsPlayer() ) // AI Patch Addition.
	{
	...
	if ( hl2_episodic.GetBool() && !GetEnemy() && HasCondition( COND_HEAR_PLAYER ) ) // AI Patch Addition.
	{
		Vector los = ( pPlayer->EyePosition() - EyePosition() ); // AI Patch Addition.
		los.z = 0;
		VectorNormalize( los );
		...
		case TASK_PC_GET_PATH_OFF_COMPANION:
			{
				//if ( AI_IsSinglePlayer() ) // AI Patch Addition.
				{
					GetNavigator()->SetAllowBigStep( UTIL_GetNearestPlayer(GetAbsOrigin()) ); // AI Patch Addition.
				}
...
void CNPC_PlayerCompanion::Touch( CBaseEntity *pOther )
{
	...
	if ( pOther->IsPlayer() || ( pOther->VPhysicsGetObject() && (pOther->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD ) ) )
	{
		...
		TestPlayerPushing( ( pOther->IsPlayer() ) ? pOther : UTIL_GetNearestPlayer(GetAbsOrigin()) ); // AI Patch Addition.
	}
}
...
void CNPC_PlayerCompanion::UpdateReadiness()
{
	...
	if ( !IsInScriptedReadinessState() )
	{
 	...
 	if( ai_debug_readiness.GetBool() ) // AI Patch Addition.
	{
...
bool CNPC_PlayerCompanion::PickTacticalLookTarget( AILookTargetArgs_t *pArgs )
{
	...
	pArgs->flDuration = random->RandomFloat( flMinLookTime, flMaxLookTime ); // AI Patch Addition.
...
bool CNPC_PlayerCompanion::Weapon_CanUse( CBaseCombatWeapon *pWeapon )
{
	if( BaseClass::Weapon_CanUse( pWeapon ) )
	{
		...
		{
			return (NumWeaponsInSquad("weapon_doublebarrel") < 1 );
		}
		else
		{
			return true;
...
//		return; // AI Patch Addition.
	...
	CBaseEntity *pPlayer = UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
...
bool CNPC_PlayerCompanion::IsNavigationUrgent( void )
{
	...
	if ( !bBase && (m_FollowBehavior.IsActive() || ( m_AssaultBehavior.IsRunning() && m_AssaultBehavior.IsUrgent() )) && Classify() == CLASS_PLAYER_ALLY_VITAL ) 
	{
		...
		{ // AI Patch Addition.
			...
			if ( pPlayer->FInViewCone( EyePosition() ) ) // AI Patch Addition.
				return false;
	} // AI Patch Addition.
