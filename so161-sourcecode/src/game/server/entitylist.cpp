...
CGlobalEntityList::CGlobalEntityList()
{
	...
	// S:O - Clear Lists
	m_SurvivorList.Purge();
	m_ZombieList.Purge();
	m_MilitaryList.Purge();
	m_PlayerList.Purge();
	m_EscapedList.Purge();
}
...
CBaseEntity *CGlobalEntityList::FindEntityProcedural( const char *szName, CBaseEntity *pSearchingEntity, CBaseEntity *pActivator, CBaseEntity *pCaller )
{
	...
	if ( szName[0] == '!' )
	{
		...
		{
			return (CBaseEntity *)UTIL_GetLocalPlayer(); // AI Patch Addition.
		}
		...
		{
			return FindPickerEntity( UTIL_GetLocalPlayer() ); // AI Patch Addition.
		}
