...
void CAI_BaseNPC::ConqCommanded(const Vector &targetPos, const Vector &traceDir, bool bRun, CAI_BaseNPC *npc) 
{
	Vector chasePosition = targetPos;
	Vector vUpBit = chasePosition;
	vUpBit.z += 1;
	...
	npc->m_vecLastPosition = chasePosition;
	npc->m_vecLastGoalPosition = chasePosition;
	...
	npc->SetCondition( COND_RECEIVED_ORDERS );
