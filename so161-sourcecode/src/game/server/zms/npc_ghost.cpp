...
//=========================================================
//=========================================================
class CGhost : public CNPC_BaseZombie
{
	DECLARE_CLASS( CGhost, CNPC_BaseZombie );
...
LINK_ENTITY_TO_CLASS( npc_ghost, CGhost );

// S:O - Connect to new client version
IMPLEMENT_SERVERCLASS_ST(CGhost, DT_Ghost)
END_SEND_TABLE()

BEGIN_DATADESC( CGhost )
...
//
//-----------------------------------------------------------------------------
void CGhost::Precache( void )
{
	PrecacheModel("models/Zombies/reaper1.mdl");

#ifdef HL2_EPISODIC
	PrecacheScriptSound( "NPC_Ghost.CarEnter1" );
	PrecacheScriptSound( "NPC_Ghost.CarEnter2" );
	PrecacheScriptSound( "NPC_Ghost.CarEnter3" );
	PrecacheScriptSound( "NPC_Ghost.CarEnter4" );
	PrecacheScriptSound( "NPC_Ghost.CarScream" );
#endif
	
	PrecacheScriptSound( "NPC_Ghost.LeapAttack" );
	PrecacheScriptSound( "NPC_Ghost.FootstepRight" );
	PrecacheScriptSound( "NPC_Ghost.FootstepLeft" );
	PrecacheScriptSound( "NPC_Ghost.AttackHit" );
	PrecacheScriptSound( "NPC_Ghost.AttackMiss" );
	PrecacheScriptSound( "NPC_Ghost.LeapAttack" );
	PrecacheScriptSound( "NPC_Ghost.Attack" );
	PrecacheScriptSound( "NPC_Ghost.Idle" );
	PrecacheScriptSound( "NPC_Ghost.AlertFar" );
	PrecacheScriptSound( "NPC_Ghost.AlertNear" );
	PrecacheScriptSound( "NPC_Ghost.GallopLeft" );
	PrecacheScriptSound( "NPC_Ghost.GallopRight" );
	PrecacheScriptSound( "NPC_Ghost.Scream" );
	PrecacheScriptSound( "NPC_Ghost.RangeAttack" );
	PrecacheScriptSound( "NPC_Ghost.Frenzy" );
	PrecacheScriptSound( "NPC_Ghost.NoSound" );
	PrecacheScriptSound( "NPC_Ghost.Die" );

	PrecacheScriptSound( "NPC_Ghost.Gurgle" );

	PrecacheScriptSound( "NPC_Ghost.Moan1" );

	// S:O - Teleport sound of doom!
	PrecacheScriptSound( "NPC_Ghost.Teleported" );
...
//---------------------------------------------------------
//---------------------------------------------------------
void CGhost::OnScheduleChange( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CGhost::SelectSchedule ( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CGhost::PrescheduleThink( void )
{
	...
	RadiusDamage( CTakeDamageInfo( this, this, 2.5f, DMG_RADIATION ), GetAbsOrigin(), 50.0f, CLASS_NONE | CLASS_ZOMBIE, NULL );
...
void CGhost::TeleportToRandomPlayer( void )
{
...
int CGhost::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
void CGhost::SoundInit( void )
{
	...
	{
		// Set up layer2
		m_pLayer2 = ENVELOPE_CONTROLLER.SoundCreate( filter, entindex(), CHAN_VOICE, "NPC_Ghost.Gurgle", ATTN_NORM );
...
//-----------------------------------------------------------------------------
void CGhost::SetIdleSoundState( void )
{
	/*// Main looping sound
	if ( m_pMoanSound )
	{
...
//-----------------------------------------------------------------------------
void CGhost::SetAngrySoundState( void )
{
	...
	EmitSound( "NPC_Ghost.LeapAttack" );
...
void CGhost::Spawn( void )
{
	...
	m_iHealth			= so_ghost_health.GetInt();
	m_flFieldOfView		= 1.0;
	...
	m_flFieldOfView	= 1.0;
	SetRenderColorA(25);
	m_nRenderFX = kRenderFxStrobeFaster;

	// S:O - The ghost does not use the dynamic difficulty system for health, even if it is enabled
	// The code will handle this on its own
	m_iHealth = so_ghost_health.GetInt();
	m_iMaxHealth = so_ghost_health.GetInt();
...
//
//-----------------------------------------------------------------------------
void CGhost::SetZombieModel( void )
{
...
//
//-----------------------------------------------------------------------------
int CGhost::MeleeAttack1Conditions( float flDot, float flDist )
{
	if ( !GetEnemy() )
	{
		return COND_NONE;
...
//-----------------------------------------------------------------------------
const char *CGhost::GetMoanSound( int nSound )
{
...
//-----------------------------------------------------------------------------
void CGhost::FootstepSound( bool fRightFoot )
{
	if( fRightFoot )
	{
		EmitSound( "NPC_Ghost.FootstepRight" );
	}
	else
	{
		EmitSound( "NPC_Ghost.FootstepLeft" );
	}
}
...
//-----------------------------------------------------------------------------
void CGhost::AttackHitSound( void )
{
	EmitSound( "NPC_Ghost.AttackHit" );
}

//-----------------------------------------------------------------------------
// Purpose: Play a random attack miss sound
//-----------------------------------------------------------------------------
void CGhost::AttackMissSound( void )
{
	// Play a random attack miss sound
	EmitSound( "NPC_Ghost.AttackMiss" );
}

//-----------------------------------------------------------------------------
// Purpose: Play a random attack sound.
//-----------------------------------------------------------------------------
void CGhost::LeapAttackSound( void )
{
	EmitSound( "NPC_Ghost.LeapAttack" );
}
...
//-----------------------------------------------------------------------------
void CGhost::PainSound( const CTakeDamageInfo &info )
{
	/*if ( m_pLayer2 )
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CGhost::DeathSound( const CTakeDamageInfo &info ) 
{
	EmitSound( "NPC_Ghost.Die" );
}
...
void CGhost::AlertSound( void )
{
	...
	if( pPlayer )
	{
		...
		if( flDist > 512 )
		{
			EmitSound( "NPC_Ghost.AlertFar" );
		}
		else
		{
			EmitSound( "NPC_Ghost.AlertNear" );
		}
	}
}
...
float CGhost::InnateRange1MaxRange( void ) 
{ 
...
//
//-----------------------------------------------------------------------------
int CGhost::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
...
//
//-----------------------------------------------------------------------------
void CGhost::HandleAnimEvent( animevent_t *pEvent )
{
	...
	{
		EmitSound( "NPC_Ghost.GallopLeft" );
		return;
	}

	if ( pEvent->event == AE_FASTZOMBIE_GALLOP_RIGHT )
	{
		EmitSound( "NPC_Ghost.GallopRight" );
		return;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGhost::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	...
	case TASK_RANGE_ATTACK1:
		...
		SetTouch( &CGhost::LeapAttackTouch );
		break;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGhost::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_FASTZOMBIE_JUMP_BACK:
...
//---------------------------------------------------------
//---------------------------------------------------------
int CGhost::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
	case SCHED_RANGE_ATTACK1:
		{
			...
			if( !m_fHasScreamed )
			{
				...
				EmitSound( "NPC_Ghost.Scream" );
				m_fHasScreamed = true;
			}
			else
			{
				EmitSound( "NPC_Ghost.RangeAttack" );
			}
...
//---------------------------------------------------------
//---------------------------------------------------------
Activity CGhost::NPC_TranslateActivity( Activity baseAct )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CGhost::LeapAttackTouch( CBaseEntity *pOther )
{
	if ( !pOther->IsSolid() )
	{
...
//-----------------------------------------------------------------------------
void CGhost::ClimbTouch( CBaseEntity *pOther )
{
	if ( pOther->IsPlayer() )
	{
...
//-----------------------------------------------------------------------------
void CGhost::StopLoopingSounds( void )
{
	/*if ( m_pMoanSound )
	{
...
// Output :
//-----------------------------------------------------------------------------
bool CGhost::IsJumpLegal(const Vector &startPos, const Vector &apex, const Vector &endPos) const
{
...
bool CGhost::MovementCost( int moveType, const Vector &vecStart, const Vector &vecEnd, float *pCost )
{
...
bool CGhost::ShouldFailNav( bool bMovementFailed )
{
...
//---------------------------------------------------------
void CGhost::OnNavJumpHitApex( void )
{
...
//			jump state.
//---------------------------------------------------------
void CGhost::OnChangeActivity( Activity NewActivity )
{
	if ( NewActivity == ACT_FASTZOMBIE_FRENZY )
	{
		// Scream!!!!
		EmitSound( "NPC_Ghost.Frenzy" );
...
// 
//=========================================================
int CGhost::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	if ( m_fJustJumped )
	{
...
// 
//=========================================================
void CGhost::BeginAttackJump( void )
{
...
// 
//=========================================================
void CGhost::EndAttackJump( void )
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGhost::BuildScheduleTestBits( void )
{
...
// 
//=========================================================
void CGhost::OnStateChange( NPC_STATE OldState, NPC_STATE NewState )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CGhost::Event_Killed( const CTakeDamageInfo &info )
{
	...
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "NPC_Ghost.NoSound" );
	StopLoopingSounds();

	// S:O - Just in case...
	StopSound( "NPC_Ghost.LeapAttack" );
	StopSound( "NPC_Ghost.FootstepRight" );
	StopSound( "NPC_Ghost.FootstepLeft" );
	StopSound( "NPC_Ghost.AttackHit" );
	StopSound( "NPC_Ghost.AttackMiss" );
	StopSound( "NPC_Ghost.LeapAttack" );
	StopSound( "NPC_Ghost.Attack" );
	StopSound( "NPC_Ghost.Idle" );
	StopSound( "NPC_Ghost.AlertFar" );
	StopSound( "NPC_Ghost.AlertNear" );
	StopSound( "NPC_Ghost.GallopLeft" );
	StopSound( "NPC_Ghost.GallopRight" );
	StopSound( "NPC_Ghost.Scream" );
	StopSound( "NPC_Ghost.RangeAttack" );
	StopSound( "NPC_Ghost.Frenzy" );
	StopSound( "NPC_Ghost.NoSound" );
	StopSound( "NPC_Ghost.Die" );
	StopSound( "NPC_Ghost.Gurgle" );
	StopSound( "NPC_Ghost.Moan1" );
	StopSound( "NPC_Ghost.Teleported" );
...
//-----------------------------------------------------------------------------
bool CGhost::CreateBehaviors( void )
{
...
//-----------------------------------------------------------------------------
void CGhost::InputAttachToVehicle( inputdata_t &inputdata )
{
	// Interrupt us
...
//-----------------------------------------------------------------------------
void CGhost::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
...
//-----------------------------------------------------------------------------
void CGhost::VehicleLeapAttack( void )
{
	CBaseEntity *pEnemy = GetEnemy();
	if ( pEnemy == NULL )
		return;
	...
	SetTouch( &CGhost::VehicleLeapAttackTouch );
}
...
//-----------------------------------------------------------------------------
bool CGhost::CanEnterVehicle( CPropJeepEpisodic *pVehicle )
{
	if ( pVehicle == NULL )
		return false;
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CGhost::VehicleLeapAttackTouch( CBaseEntity *pOther )
{
...
//-----------------------------------------------------------------------------
void CGhost::UpdateEfficiency( bool bInPVS )
{ 
...
AI_BEGIN_CUSTOM_NPC( npc_ghost, CGhost )
