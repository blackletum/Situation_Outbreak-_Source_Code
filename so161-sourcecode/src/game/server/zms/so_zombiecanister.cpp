...
// Models!
//-----------------------------------------------------------------------------
#define ENV_ZombieCANISTER_MODEL	"models/props_combine/headcrabcannister01a.mdl"
#define ENV_ZombieCANISTER_BROKEN_MODEL	"models/props_combine/headcrabcannister01b.mdl"
#define ENV_ZombieCANISTER_SKYBOX_MODEL	"models/props_combine/headcrabcannister01a_skybox.mdl"
#define ENV_ZombieCANISTER_INCOMING_SOUND_TIME	1.0f

ConVar sk_env_Zombiecanister_shake_amplitude( "sk_env_Zombiecanister_shake_amplitude", "50" );
ConVar sk_env_Zombiecanister_shake_radius( "sk_env_Zombiecanister_shake_radius", "1024" );
ConVar sk_env_Zombiecanister_shake_radius_vehicle( "sk_env_Zombiecanister_shake_radius_vehicle", "2500" );

#define ENV_ZombieCANISTER_TRAIL_TIME	3.0f
...
enum
{
	...
	SF_WAIT_FOR_INPUT_TO_SPAWN_ZombieS = 0x8000,
	SF_NO_SMOKE	= 0x10000,
	SF_NO_SHAKE = 0x20000,
...
// Zombie types
//-----------------------------------------------------------------------------
static const char *s_pZombieClass[] = 
{
	"npc_zombie",
	"npc_poisonzombie",
	"npc_fastzombie",
...
static const char *s_pZombieThinkContext = "ZombieThink";


//=============================================================================
//
// ZombieCanister Functions
//

LINK_ENTITY_TO_CLASS( so_zombiecanister, CZombieCanister );

BEGIN_DATADESC( CZombieCanister )

	DEFINE_FIELD( m_bLanded,							FIELD_BOOLEAN ),
	DEFINE_EMBEDDED( m_Shared ),
	DEFINE_FIELD( m_hTrail,								FIELD_EHANDLE ),
	DEFINE_FIELD( m_hSmokeTrail,						FIELD_EHANDLE ),
	DEFINE_KEYFIELD( m_nZombieType,					FIELD_INTEGER,	"ZombieType" ),
	DEFINE_KEYFIELD( m_nZombieCount,					FIELD_INTEGER,	"ZombieCount" ),
	...
	// Function Pointers.
	DEFINE_FUNCTION( ZombieCanisterSkyboxThink ),
	DEFINE_FUNCTION( ZombieCanisterWorldThink ),
	DEFINE_FUNCTION( ZombieCanisterSpawnZombieThink ),
	DEFINE_FUNCTION( WaitForOpenSequenceThink ),
	DEFINE_FUNCTION( ZombieCanisterSkyboxOnlyThink ),
	DEFINE_FUNCTION( ZombieCanisterSkyboxRestartThink ),

	// Inputs
	DEFINE_INPUTFUNC( FIELD_VOID, "FireCanister", InputFireCanister ),
	DEFINE_INPUTFUNC( FIELD_VOID, "OpenCanister", InputOpenCanister ),
	DEFINE_INPUTFUNC( FIELD_VOID, "SpawnZombies", InputSpawnZombies ),
...
IMPLEMENT_SERVERCLASS_ST( CZombieCanister, DT_EnvZombieCanister )
...
// Constructor
//-----------------------------------------------------------------------------
CZombieCanister::CZombieCanister()
{
...
// Precache!
//-----------------------------------------------------------------------------
void CZombieCanister::Precache( void )
{
	BaseClass::Precache();
	PrecacheModel( ENV_ZombieCANISTER_MODEL );
	PrecacheModel( ENV_ZombieCANISTER_BROKEN_MODEL );
	PrecacheModel( ENV_ZombieCANISTER_SKYBOX_MODEL );
	...
	UTIL_PrecacheOther( s_pZombieClass[m_nZombieType] );
}
...
// Spawn!
//-----------------------------------------------------------------------------
void CZombieCanister::Spawn( void )
{
	Precache();
	BaseClass::Spawn();
...
// On remove!
//-----------------------------------------------------------------------------
void CZombieCanister::UpdateOnRemove()
{
...
//-----------------------------------------------------------------------------
void CZombieCanister::SetupWorldModel()
{
	SetModel( ENV_ZombieCANISTER_MODEL );
	SetSolid( SOLID_BBOX );
...
//-----------------------------------------------------------------------------
void CZombieCanister::ComputeWorldEntryPoint( Vector *pStartPosition, QAngle *pStartAngles, Vector *pStartDirection )
{
	SetupWorldModel();
...
//-----------------------------------------------------------------------------
CSkyCamera *CZombieCanister::PlaceCanisterInWorld()
{
	...
	if ( m_iszLaunchPositionName != NULL_STRING )
	{
		...
		else
		{
			...
			SetThink( &CZombieCanister::ZombieCanisterWorldThink );
	...
	else if ( DetectInSkybox() )
	{
		...
		SetModel( ENV_ZombieCANISTER_SKYBOX_MODEL );
		SetSolid( SOLID_NONE );
		...
		SetThink( &CZombieCanister::ZombieCanisterSkyboxOnlyThink );
	...
	else
	{
		...
		if ( pCamera )
		{
			...
			if ( m_Shared.IsInSkybox() )
			{
				...
				SetSolid( SOLID_NONE );
				AddEFlags( EFL_IN_SKYBOX );
				SetThink( &CZombieCanister::ZombieCanisterSkyboxThink );
...
// Fires the canister!
//-----------------------------------------------------------------------------
void CZombieCanister::InputFireCanister( inputdata_t &inputdata )
{
	if (m_bLaunched)
		return;
	...
	m_hTrail->SetLifeTime( ENV_ZombieCANISTER_TRAIL_TIME );
...
// Opens the canister!
//-----------------------------------------------------------------------------
void CZombieCanister::InputOpenCanister( inputdata_t &inputdata )
{
...
// Spawns Zombies
//-----------------------------------------------------------------------------
void CZombieCanister::InputSpawnZombies( inputdata_t &inputdata )
{
	if ( m_bLanded && m_bOpened && HasSpawnFlags( SF_WAIT_FOR_INPUT_TO_SPAWN_ZombieS ) )
	{
		StartSpawningZombies( 0.01f );
	}
}
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CZombieCanister::InputStopSmoke( inputdata_t &inputdata )
{
...
// Test for impact!
//-----------------------------------------------------------------------------
void CZombieCanister::TestForCollisionsAgainstEntities( const Vector &vecEndPosition )
{
	// Debugging!!
...
void CZombieCanister::TestForCollisionsAgainstWorld( const Vector &vecEndPosition )
{
	// Splash damage!
...
// Zombie creation
//-----------------------------------------------------------------------------
void CZombieCanister::ZombieCanisterSpawnZombieThink()
{
	...
	int nZombieAttachment = LookupAttachment( "Headcrab" );
	if ( GetAttachment( nZombieAttachment, vecSpawnPosition, vecSpawnAngles ) )
	{
		CBaseEntity *pEnt = CreateEntityByName( s_pZombieClass[m_nZombieType] );
		CNPC_BaseZombie *pZombie = assert_cast<CNPC_BaseZombie*>(pEnt);
		...
		pZombie->SetParent( this, nZombieAttachment );
		pZombie->SetLocalOrigin( vec3_origin );
		pZombie->SetLocalAngles( vec3_angle );
		//pZombie->CrawlFromCanister();
	}

	if ( m_nZombieCount != 0 )
	{
		float flWaitTime = random->RandomFloat( 1.0f, 2.0f );
		SetContextThink( &CZombieCanister::ZombieCanisterSpawnZombieThink, gpGlobals->curtime + flWaitTime, s_pZombieThinkContext );
	}
	else
	{
		SetContextThink( NULL, gpGlobals->curtime, s_pZombieThinkContext );
	}
}
...
//-----------------------------------------------------------------------------
void CZombieCanister::StartSpawningZombies( float flDelay )
{
	if ( !m_bLanded || !m_bOpened || m_nZombieCount == 0 )
		return;

	if ( m_nZombieCount != 0 )
	{
		SetContextThink( &CZombieCanister::ZombieCanisterSpawnZombieThink, gpGlobals->curtime + flDelay, s_pZombieThinkContext );
	}
}
...
//-----------------------------------------------------------------------------
void CZombieCanister::CanisterFinishedOpening( void )
{
	...
	if ( !HasSpawnFlags( SF_START_IMPACTED ) )
	{
		if ( !HasSpawnFlags( SF_WAIT_FOR_INPUT_TO_SPAWN_ZombieS ) )
		{
			StartSpawningZombies( 3.0f );
		}
	}
}
...
//-----------------------------------------------------------------------------
void CZombieCanister::WaitForOpenSequenceThink()
{
	StudioFrameAdvance();
	...
	{
		SetContextThink( &CZombieCanister::WaitForOpenSequenceThink, gpGlobals->curtime + 0.01f, s_pOpenThinkContext );
	}
}
...
// Open the canister!
//-----------------------------------------------------------------------------
void CZombieCanister::OpenCanister( void )
{
	if ( m_bOpened )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CZombieCanister::SetLanded( void )
{
	SetAbsOrigin( m_vecImpactPosition );
	SetModel( ENV_ZombieCANISTER_BROKEN_MODEL );
...
// Landed!
//-----------------------------------------------------------------------------
void CZombieCanister::Landed( void )
{
	EmitSound( "ZombieCanister.AfterLanding" );
	...
	{
		// Create the smoke trail to obscure the Zombies
...
void CZombieCanister::Detonate( )
{
	...
	if ( HasSpawnFlags( SF_REMOVE_ON_IMPACT ) )
	{
		...
		SetThink( &CZombieCanister::SUB_Remove );
		SetNextThink( gpGlobals->curtime + ENV_ZombieCANISTER_TRAIL_TIME );
	...
	if ( !HasSpawnFlags( SF_NO_SHAKE ) )
	{
		...
		float shakeRadius = ( pPlayer && pPlayer->IsInAVehicle() ) ? sk_env_Zombiecanister_shake_radius_vehicle.GetFloat() : sk_env_Zombiecanister_shake_radius.GetFloat();

		UTIL_ScreenShake( m_vecImpactPosition, sk_env_Zombiecanister_shake_amplitude.GetFloat(), 150.0, 1.0, shakeRadius, SHAKE_START );
	}
...
//-----------------------------------------------------------------------------
// Purpose: This think function simulates (moves/collides) the ZombieCanister while in
//          the world.
//-----------------------------------------------------------------------------
void CZombieCanister::ZombieCanisterWorldThink( void )
{
	...
	// Update ZombieCanister position for swept collision test.
	Vector vecEndPosition;
	QAngle vecEndAngles;
	...
	{
		float flDistSq = ENV_ZombieCANISTER_INCOMING_SOUND_TIME * m_Shared.m_flFlightSpeed;
		flDistSq *= flDistSq;
...
//-----------------------------------------------------------------------------
// Purpose: This think function should be called at the time when the ZombieCanister 
//          will be leaving the skybox and entering the world.
//-----------------------------------------------------------------------------
void CZombieCanister::ZombieCanisterSkyboxThink( void )
{
...
//-----------------------------------------------------------------------------
void CZombieCanister::ZombieCanisterSkyboxOnlyThink( void )
{
	Vector vecEndPosition;
	QAngle vecEndAngles;
	m_Shared.GetPositionAtTime( gpGlobals->curtime, vecEndPosition, vecEndAngles );
	...
	float flRefireTime = random->RandomFloat( m_flMinRefireTime, m_flMaxRefireTime ) + ENV_ZombieCANISTER_TRAIL_TIME;
	SetThink( &CZombieCanister::ZombieCanisterSkyboxRestartThink );
	SetNextThink( gpGlobals->curtime + flRefireTime );
}


//-----------------------------------------------------------------------------
// This will re-fire the Zombie cannister
//-----------------------------------------------------------------------------
void CZombieCanister::ZombieCanisterSkyboxRestartThink( void )
{
	if ( m_hTrail )
	{
		UTIL_Remove( m_hTrail );
...
//			bAlways - 
//-----------------------------------------------------------------------------
void CZombieCanister::SetTransmit( CCheckTransmitInfo *pInfo, bool bAlways )
{
