...
extern ConVar so_reaper_nojump_health;
extern ConVar so_reaper_nojump_health_dd;
...
/*envelopePoint_t envFastZombieVolumeJump2[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envFastZombieVolumePain2[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envFastZombieInverseVolumePain2[] =
{
	{	0.0f, 0.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envFastZombieVolumeJump2PostApex2[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envFastZombieVolumeClimb2[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envFastZombieMoanVolume2Fast2[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envFastZombieMoanVolume2[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envFastZombieFootstepVolume2[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envFastZombieVolumeFrenzy2[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
//=========================================================
//=========================================================
class CReaperNoJump : public CNPC_BaseZombie
{
	DECLARE_CLASS( CReaperNoJump, CNPC_BaseZombie );
...
LINK_ENTITY_TO_CLASS( npc_reaper_nojump, CReaperNoJump );

// S:O - Connect to new client version
IMPLEMENT_SERVERCLASS_ST(CReaperNoJump, DT_ReaperNoJump)
END_SEND_TABLE()

BEGIN_DATADESC( CReaperNoJump )
...
const char *CReaperNoJump::pMoanSounds[] =
{
...
//
//-----------------------------------------------------------------------------
void CReaperNoJump::Precache( void )
{
	PrecacheModel("models/Zombies/reaper1.mdl");
	PrecacheModel("models/Zombies/reaper2.mdl");
	PrecacheModel("models/Zombies/reaper3.mdl");
	PrecacheModel("models/Zombies/reaper4.mdl");
	PrecacheModel("models/Zombies/reaper5.mdl");
	PrecacheModel("models/Zombies/reaper6.mdl");
	PrecacheModel("models/Zombies/reaper7.mdl");
	PrecacheModel("models/Zombies/reaper8.mdl");
	PrecacheModel("models/Zombies/reaper9.mdl");
	PrecacheModel("models/Zombies/reaper10.mdl");
	PrecacheModel("models/Zombies/reaper11.mdl");
	PrecacheModel("models/Zombies/reaper12.mdl");
	PrecacheModel("models/Zombies/reaper13.mdl");
	PrecacheModel("models/Zombies/reaper14.mdl");
	PrecacheModel("models/Zombies/reaper15.mdl");
	PrecacheModel("models/Zombies/reaper16.mdl");
	PrecacheModel("models/Zombies/reaper17.mdl");
#ifdef HL2_EPISODIC
...
//---------------------------------------------------------
//---------------------------------------------------------
void CReaperNoJump::OnScheduleChange( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CReaperNoJump::SelectSchedule ( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CReaperNoJump::PrescheduleThink( void )
{
...
int CReaperNoJump::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
//
//-----------------------------------------------------------------------------
void CReaperNoJump::SoundInit( void )
{
	/*if( !m_pMoanSound )
	{
		// !!!HACKHACK - kickstart the moan sound. (sjb)
		MoanSound( envFastZombieMoanVolume2, ARRAYSIZE( envFastZombieMoanVolume2 ) );
...
//-----------------------------------------------------------------------------
void CReaperNoJump::SetIdleSoundState( void )
{
	// Main looping sound
	/*if ( m_pMoanSound )
	{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::SetAngrySoundState( void )
{
...
void CReaperNoJump::Spawn( void )
{
	...
	m_iHealth			= so_reaper_nojump_health.GetInt();
	m_flFieldOfView		= 0.2;
	...
	// S:O - We don't want non-jumping reapers to be able to jump (for obvious reasons)
	...
	{
		m_iHealth = so_reaper_nojump_health_dd.GetInt();
		m_iMaxHealth = so_reaper_nojump_health_dd.GetInt();
	}
	else
	{
		m_iHealth = so_reaper_nojump_health.GetInt();
		m_iMaxHealth = so_reaper_nojump_health.GetInt();
	}
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CReaperNoJump::PostNPCInit( void )
{
	SoundInit();
...
// Purpose: 
//-----------------------------------------------------------------------------
float CReaperNoJump::MaxYawSpeed( void )
{
	switch( GetActivity() )
	{
	case ACT_TURN_LEFT:
...
//
//-----------------------------------------------------------------------------
void CReaperNoJump::SetZombieModel( void )
{
	Hull_t lastHull = GetHullType();

	int randfastzombie = RandomInt(1,17);
	switch ( randfastzombie )
	{
		case 1:
		...
		case 2:
			SetModel( "models/Zombies/reaper2.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 3:
			SetModel( "models/Zombies/reaper3.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 4:
			SetModel( "models/Zombies/reaper4.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 5:
			SetModel( "models/Zombies/reaper5.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 6:
			SetModel( "models/Zombies/reaper6.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 7:
			SetModel( "models/Zombies/reaper7.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 8:
			SetModel( "models/Zombies/reaper8.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 9:
			SetModel( "models/Zombies/reaper9.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 10:
			SetModel( "models/Zombies/reaper10.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 11:
			SetModel( "models/Zombies/reaper11.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 12:
			SetModel( "models/Zombies/reaper12.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 13:
			SetModel( "models/Zombies/reaper13.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 14:
			SetModel( "models/Zombies/reaper14.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 15:
			SetModel( "models/Zombies/reaper15.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 16:
			SetModel( "models/Zombies/reaper16.mdl" );
			SetHullType(HULL_HUMAN);
			break;

		case 17:
			SetModel( "models/Zombies/reaper17.mdl" );
			SetHullType(HULL_HUMAN);
			break;
	}
...
//
//-----------------------------------------------------------------------------
int CReaperNoJump::MeleeAttack1Conditions( float flDot, float flDist )
{
	if ( !GetEnemy() )
	{
		return COND_NONE;
...
//-----------------------------------------------------------------------------
const char *CReaperNoJump::GetMoanSound( int nSound )
{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::FootstepSound( bool fRightFoot )
{
	if( fRightFoot )
	{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::AttackHitSound( void )
{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::AttackMissSound( void )
{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::LeapAttackSound( void )
{
	EmitSound( "NPC_FastZombie.LeapAttack" );
}
...
// Purpose: Play a random attack sound.
//-----------------------------------------------------------------------------
void CReaperNoJump::AttackSound( void )
{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::IdleSound( void )
{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::PainSound( const CTakeDamageInfo &info )
{
	/*if ( m_pLayer2 )
		ENVELOPE_CONTROLLER.SoundPlayEnvelope( m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envFastZombieVolumePain2, ARRAYSIZE(envFastZombieVolumePain2) );
	if ( m_pMoanSound )
		ENVELOPE_CONTROLLER.SoundPlayEnvelope( m_pMoanSound, SOUNDCTRL_CHANGE_VOLUME, envFastZombieInverseVolumePain2, ARRAYSIZE(envFastZombieInverseVolumePain2) );*/
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CReaperNoJump::DeathSound( const CTakeDamageInfo &info ) 
{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::AlertSound( void )
{
...
float CReaperNoJump::InnateRange1MaxRange( void ) 
{ 
...
//
//-----------------------------------------------------------------------------
int CReaperNoJump::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
	...
	vecDirToEnemy = vecDirToEnemy * 0.25;	// S:O - reduced this a bit from 0.5
...
//
//-----------------------------------------------------------------------------
void CReaperNoJump::HandleAnimEvent( animevent_t *pEvent )
{
	if ( pEvent->event == AE_FASTZOMBIE_CLIMB_LEFT || pEvent->event == AE_FASTZOMBIE_CLIMB_RIGHT )
	{
		/*if( ++m_iClimbCount % 3 == 0 )
		{
			...
			ENVELOPE_CONTROLLER.SoundPlayEnvelope( m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envFastZombieVolumeClimb2, ARRAYSIZE(envFastZombieVolumeClimb2) );
		}*/
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_RIGHT )
	{
		...
		ClawAttack( GetClawAttackRange(), random->RandomFloat( 1, 2.5 ), QAngle( -3, -5, -3 ), right, ZOMBIE_BLOOD_RIGHT_HAND );
		return;
	}
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_LEFT )
	{
		...
		AngleVectors( GetLocalAngles(), NULL, &right, NULL );
		right = right * 50;
		ClawAttack( GetClawAttackRange(), random->RandomFloat( 1, 2.5 ), QAngle( -3, 5, -3 ), right, ZOMBIE_BLOOD_LEFT_HAND );
		return;
	}
...
//
//-----------------------------------------------------------------------------
void CReaperNoJump::LeapAttack( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CReaperNoJump::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	...
	case TASK_RANGE_ATTACK1:
		...
		SetTouch( &CReaperNoJump::LeapAttackTouch );
		break;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CReaperNoJump::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_FASTZOMBIE_JUMP_BACK:
...
//---------------------------------------------------------
//---------------------------------------------------------
int CReaperNoJump::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
Activity CReaperNoJump::NPC_TranslateActivity( Activity baseAct )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CReaperNoJump::LeapAttackTouch( CBaseEntity *pOther )
{
	if ( !pOther->IsSolid() )
	{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::ClimbTouch( CBaseEntity *pOther )
{
	if ( pOther->IsPlayer() )
	{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::StopLoopingSounds( void )
{
	/*if ( m_pMoanSound )
	{
...
// Output :
//-----------------------------------------------------------------------------
bool CReaperNoJump::IsJumpLegal(const Vector &startPos, const Vector &apex, const Vector &endPos) const
{
...
bool CReaperNoJump::MovementCost( int moveType, const Vector &vecStart, const Vector &vecEnd, float *pCost )
{
...
bool CReaperNoJump::ShouldFailNav( bool bMovementFailed )
{
...
//---------------------------------------------------------
void CReaperNoJump::OnNavJumpHitApex( void )
{
...
//			jump state.
//---------------------------------------------------------
void CReaperNoJump::OnChangeActivity( Activity NewActivity )
{
	...
	if ( NewActivity == ACT_CLIMB_UP )
	{
		...
		SetTouch( &CReaperNoJump::ClimbTouch );
	}
...
// 
//=========================================================
int CReaperNoJump::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	if ( m_fJustJumped )
	{
...
//=========================================================
void CReaperNoJump::BeginNavJump( void )
{
	m_fIsNavJumping = true;
	m_fHitApex = false;

	//ENVELOPE_CONTROLLER.SoundPlayEnvelope( m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envFastZombieVolumeJump2, ARRAYSIZE(envFastZombieVolumeJump2) );
}

//=========================================================
// 
//=========================================================
void CReaperNoJump::EndNavJump( void )
{
...
// 
//=========================================================
void CReaperNoJump::BeginAttackJump( void )
{
...
// 
//=========================================================
void CReaperNoJump::EndAttackJump( void )
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CReaperNoJump::BuildScheduleTestBits( void )
{
...
// 
//=========================================================
void CReaperNoJump::OnStateChange( NPC_STATE OldState, NPC_STATE NewState )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CReaperNoJump::Event_Killed( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
bool CReaperNoJump::CreateBehaviors( void )
{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::InputAttachToVehicle( inputdata_t &inputdata )
{
	// Interrupt us
...
//-----------------------------------------------------------------------------
void CReaperNoJump::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::VehicleLeapAttack( void )
{
	CBaseEntity *pEnemy = GetEnemy();
	if ( pEnemy == NULL )
		return;
	...
	SetTouch( &CReaperNoJump::VehicleLeapAttackTouch );
}
...
//-----------------------------------------------------------------------------
bool CReaperNoJump::CanEnterVehicle( CPropJeepEpisodic *pVehicle )
{
	if ( pVehicle == NULL )
		return false;
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CReaperNoJump::VehicleLeapAttackTouch( CBaseEntity *pOther )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CReaperNoJump::IsInAVehicle( void )
{
...
//-----------------------------------------------------------------------------
void CReaperNoJump::UpdateEfficiency( bool bInPVS )
{ 
...
AI_BEGIN_CUSTOM_NPC( npc_reaper_nojump, CReaperNoJump )
