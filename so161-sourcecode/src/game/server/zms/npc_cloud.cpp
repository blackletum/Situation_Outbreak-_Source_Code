...
/*envelopePoint_t envCloudVolumeJump3[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envCloudVolumePain3[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envCloudInverseVolumePain3[] =
{
	{	0.0f, 0.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envCloudVolumeJump3PostApex3[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envCloudVolumeClimb3[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envCloudMoanVolume3Fast3[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envCloudMoanVolume3[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envCloudFootstepVolume3[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
envelopePoint_t envCloudVolumeFrenzy3[] =
{
	{	1.0f, 1.0f,
		0.1f, 0.1f,
	},
...
//=========================================================
//=========================================================
class CCloud : public CNPC_BaseZombie
{
	DECLARE_CLASS( CCloud, CNPC_BaseZombie );
...
LINK_ENTITY_TO_CLASS( npc_cloud, CCloud );

// S:O - Connect to new client version
IMPLEMENT_SERVERCLASS_ST(CCloud, DT_Cloud)
END_SEND_TABLE()

BEGIN_DATADESC( CCloud )
...
//
//-----------------------------------------------------------------------------
void CCloud::Precache( void )
{
	PrecacheModel("models/Zombies/reaper1.mdl");

#ifdef HL2_EPISODIC
	PrecacheScriptSound( "NPC_Cloud.CarEnter1" );
	PrecacheScriptSound( "NPC_Cloud.CarEnter2" );
	PrecacheScriptSound( "NPC_Cloud.CarEnter3" );
	PrecacheScriptSound( "NPC_Cloud.CarEnter4" );
	PrecacheScriptSound( "NPC_Cloud.CarScream" );
#endif
	
	PrecacheScriptSound( "NPC_Cloud.LeapAttack" );
	PrecacheScriptSound( "NPC_Cloud.FootstepRight" );
	PrecacheScriptSound( "NPC_Cloud.FootstepLeft" );
	PrecacheScriptSound( "NPC_Cloud.AttackHit" );
	PrecacheScriptSound( "NPC_Cloud.AttackMiss" );
	PrecacheScriptSound( "NPC_Cloud.LeapAttack" );
	PrecacheScriptSound( "NPC_Cloud.Attack" );
	PrecacheScriptSound( "NPC_Cloud.Idle" );
	PrecacheScriptSound( "NPC_Cloud.AlertFar" );
	PrecacheScriptSound( "NPC_Cloud.AlertNear" );
	PrecacheScriptSound( "NPC_Cloud.GallopLeft" );
	PrecacheScriptSound( "NPC_Cloud.GallopRight" );
	PrecacheScriptSound( "NPC_Cloud.Scream" );
	PrecacheScriptSound( "NPC_Cloud.RangeAttack" );
	PrecacheScriptSound( "NPC_Cloud.Frenzy" );
	PrecacheScriptSound( "NPC_Cloud.NoSound" );
	PrecacheScriptSound( "NPC_Cloud.Die" );

	PrecacheScriptSound( "NPC_Cloud.Gurgle" );

	PrecacheScriptSound( "NPC_Cloud.Moan1" );

	// S:O - Teleport sound of doom!
	PrecacheScriptSound( "NPC_Cloud.Teleported" );
...
//---------------------------------------------------------
//---------------------------------------------------------
void CCloud::OnScheduleChange( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CCloud::SelectSchedule ( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCloud::PrescheduleThink( void )
{
	...
	// S:O - Deal damage to anyone who might be around us except zombies and spectators
	RadiusDamage( CTakeDamageInfo( this, this, 1.0f, DMG_NERVEGAS ), GetAbsOrigin(), 100.0f, CLASS_NONE | CLASS_ZOMBIE, NULL );

	// S:O - Attempt to find a nearby player to annoy
	if ( gpGlobals->curtime >= m_flFindPlayer )
	{
		...
		if ( pPlayer )
		{
			...
			Vector forward;
			Vector vecGoal = pPlayer->GetAbsOrigin();
			CAI_BaseNPC::ConqCommanded( vecGoal, forward, true, this );

			m_flFindPlayer = gpGlobals->curtime + 3.0f;
		}
		else
		{
			// We failed to locate our previous victim!
			// FIND ANOTHER ONE! >=D
			m_bFoundVictim = false;
		}
	}

	// S:O - Attempt to teleport near a player if we haven't already
	if ( !m_bFoundVictim )
	{
...
void CCloud::TeleportToRandomPlayer( void )
{
	...
	if ( possibleVictims.Count() > 0 )
	{
		...
		if ( pPlayerVictim && pPlayerVictim->GetTeamNumber() == 3 && pPlayerVictim->IsAlive() )
		{
			...
			Teleport( &pPlayerVictim->GetAbsOrigin(), &angles, &pPlayerVictim->GetAbsVelocity() );
...
int CCloud::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
//
//-----------------------------------------------------------------------------
void CCloud::SoundInit( void )
{
	/*if( !m_pMoanSound )
	{
		// !!!HACKHACK - kickstart the moan sound. (sjb)
		MoanSound( envCloudMoanVolume3, ARRAYSIZE( envCloudMoanVolume3 ) );
	...
	{
		// Set up layer2
		m_pLayer2 = ENVELOPE_CONTROLLER.SoundCreate( filter, entindex(), CHAN_VOICE, "NPC_Cloud.Gurgle", ATTN_NORM );
...
//-----------------------------------------------------------------------------
void CCloud::SetIdleSoundState( void )
{
	/*// Main looping sound
	if ( m_pMoanSound )
	{
...
//-----------------------------------------------------------------------------
void CCloud::SetAngrySoundState( void )
{
	...
	EmitSound( "NPC_Cloud.LeapAttack" );
...
void CCloud::Spawn( void )
{
	...
	m_iHealth			= so_cloud_health.GetInt();
	m_flFieldOfView		= 1.0;
	...
	// S:O - Call these to override our baseclass
	AddEffects( EF_NODRAW );
	SetCollisionGroup( COLLISION_GROUP_DEBRIS );
	SetBloodColor( DONT_BLEED );
	m_flFieldOfView	= 1.0;

	// S:O - The cloud does not use the dynamic difficulty system for health, even if it is enabled
	// The code will handle this on its own
	m_iHealth = so_cloud_health.GetInt();
	m_iMaxHealth = so_cloud_health.GetInt();

	SetSquadName( AllocPooledString("zomg") );
	SetSchedule( SCHED_ZOMBIE_WANDER_MEDIUM );
	...
	if ( m_pSmokeCloud )
	{
		...
		m_pSmokeCloud->m_StartSize = 100;
		m_pSmokeCloud->m_EndSize = 2;
		m_pSmokeCloud->m_Rate = 5;
	...
	// S:O - Spawn our red smoke trail
	SmokeTrail *m_pSmokeTrail = SmokeTrail::CreateSmokeTrail();
	if( m_pSmokeTrail )
	{
		m_pSmokeTrail->m_SpawnRate = 10;
		m_pSmokeTrail->m_ParticleLifetime = 1.0f;
		
		m_pSmokeTrail->m_StartColor.Init( 1.0f, 0.5f, 0.5f );
		m_pSmokeTrail->m_EndColor.Init( 0.0, 0.0, 0.0 );
		
		m_pSmokeTrail->m_StartSize = 5;
		m_pSmokeTrail->m_EndSize = 10;
		m_pSmokeTrail->m_SpawnRadius = 5;
		m_pSmokeTrail->m_MinSpeed = 8;
		m_pSmokeTrail->m_MaxSpeed = 16;
		m_pSmokeTrail->m_Opacity = 0.25f;

		m_pSmokeTrail->SetLifetime( -1 );
		m_pSmokeTrail->FollowEntity( this, "chest" );
	}
...
//
//-----------------------------------------------------------------------------
void CCloud::SetZombieModel( void )
{
	Hull_t lastHull = GetHullType();

	// S:O - This NPC is invisible anyway, so it doesn't really matter!
	SetModel( "models/Zombies/reaper1.mdl" );
...
//
//-----------------------------------------------------------------------------
int CCloud::MeleeAttack1Conditions( float flDot, float flDist )
{
	if ( !GetEnemy() )
	{
		return COND_NONE;
...
//-----------------------------------------------------------------------------
const char *CCloud::GetMoanSound( int nSound )
{
...
//-----------------------------------------------------------------------------
void CCloud::FootstepSound( bool fRightFoot )
{
	if( fRightFoot )
	{
		EmitSound( "NPC_Cloud.FootstepRight" );
	}
	else
	{
		EmitSound( "NPC_Cloud.FootstepLeft" );
	}
}
...
//-----------------------------------------------------------------------------
void CCloud::AttackHitSound( void )
{
	EmitSound( "NPC_Cloud.AttackHit" );
}

//-----------------------------------------------------------------------------
// Purpose: Play a random attack miss sound
//-----------------------------------------------------------------------------
void CCloud::AttackMissSound( void )
{
	// Play a random attack miss sound
	EmitSound( "NPC_Cloud.AttackMiss" );
}

//-----------------------------------------------------------------------------
// Purpose: Play a random attack sound.
//-----------------------------------------------------------------------------
void CCloud::LeapAttackSound( void )
{
	EmitSound( "NPC_Cloud.LeapAttack" );
}
...
// Purpose: Play a random attack sound.
//-----------------------------------------------------------------------------
void CCloud::AttackSound( void )
{
	EmitSound( "NPC_Cloud.Attack" );
}

//-----------------------------------------------------------------------------
// Purpose: Play a random idle sound.
//-----------------------------------------------------------------------------
void CCloud::IdleSound( void )
{
	EmitSound( "NPC_Cloud.Idle" );
...
//-----------------------------------------------------------------------------
void CCloud::PainSound( const CTakeDamageInfo &info )
{
	/*if ( m_pLayer2 )
		ENVELOPE_CONTROLLER.SoundPlayEnvelope( m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envCloudVolumePain3, ARRAYSIZE(envCloudVolumePain3) );
	if ( m_pMoanSound )
		ENVELOPE_CONTROLLER.SoundPlayEnvelope( m_pMoanSound, SOUNDCTRL_CHANGE_VOLUME, envCloudInverseVolumePain3, ARRAYSIZE(envCloudInverseVolumePain3) );*/
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCloud::DeathSound( const CTakeDamageInfo &info ) 
{
	EmitSound( "NPC_Cloud.Die" );
}

//-----------------------------------------------------------------------------
// Purpose: Play a random alert sound.
//-----------------------------------------------------------------------------
void CCloud::AlertSound( void )
{
	...
	if( pPlayer )
	{
		...
		if( flDist > 512 )
		{
			EmitSound( "NPC_Cloud.AlertFar" );
		}
		else
		{
			EmitSound( "NPC_Cloud.AlertNear" );
		}
	}
}
...
float CCloud::InnateRange1MaxRange( void ) 
{ 
...
//
//-----------------------------------------------------------------------------
int CCloud::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
...
//
//-----------------------------------------------------------------------------
void CCloud::HandleAnimEvent( animevent_t *pEvent )
{
	if ( pEvent->event == AE_FASTZOMBIE_CLIMB_LEFT || pEvent->event == AE_FASTZOMBIE_CLIMB_RIGHT )
	{
		/*if( ++m_iClimbCount % 3 == 0 )
		{
			...
			ENVELOPE_CONTROLLER.SoundPlayEnvelope( m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envCloudVolumeClimb3, ARRAYSIZE(envCloudVolumeClimb3) );
		}*/
	...
	{
		EmitSound( "NPC_Cloud.GallopLeft" );
		return;
	}

	if ( pEvent->event == AE_FASTZOMBIE_GALLOP_RIGHT )
	{
		EmitSound( "NPC_Cloud.GallopRight" );
		return;
	}
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_RIGHT )
	{
		...
		ClawAttack( GetClawAttackRange(), random->RandomFloat( 1, 5 ), QAngle( -3, -5, -3 ), right, ZOMBIE_BLOOD_RIGHT_HAND );
		return;
	}
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_LEFT )
	{
		...
		AngleVectors( GetLocalAngles(), NULL, &right, NULL );
		right = right * 50;
		ClawAttack( GetClawAttackRange(), random->RandomFloat( 1, 5 ), QAngle( -3, 5, -3 ), right, ZOMBIE_BLOOD_LEFT_HAND );
		return;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCloud::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	...
	case TASK_RANGE_ATTACK1:
		...
		SetTouch( &CCloud::LeapAttackTouch );
		break;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCloud::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_FASTZOMBIE_JUMP_BACK:
...
//---------------------------------------------------------
//---------------------------------------------------------
int CCloud::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
	case SCHED_RANGE_ATTACK1:
		{
			...
			if( !m_fHasScreamed )
			{
				...
				EmitSound( "NPC_Cloud.Scream" );
				m_fHasScreamed = true;
			}
			else
			{
				EmitSound( "NPC_Cloud.RangeAttack" );
			}
...
//---------------------------------------------------------
//---------------------------------------------------------
Activity CCloud::NPC_TranslateActivity( Activity baseAct )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CCloud::LeapAttackTouch( CBaseEntity *pOther )
{
	if ( !pOther->IsSolid() )
	{
	...
	ClawAttack( GetClawAttackRange(), random->RandomFloat( 1, 2.5 ), qaPunch, forward * 500, ZOMBIE_BLOOD_BOTH_HANDS );
...
//-----------------------------------------------------------------------------
void CCloud::ClimbTouch( CBaseEntity *pOther )
{
	if ( pOther->IsPlayer() )
	{
...
//-----------------------------------------------------------------------------
void CCloud::StopLoopingSounds( void )
{
	/*if ( m_pMoanSound )
	{
...
// Output :
//-----------------------------------------------------------------------------
bool CCloud::IsJumpLegal(const Vector &startPos, const Vector &apex, const Vector &endPos) const
{
...
bool CCloud::MovementCost( int moveType, const Vector &vecStart, const Vector &vecEnd, float *pCost )
{
...
bool CCloud::ShouldFailNav( bool bMovementFailed )
{
...
//---------------------------------------------------------
void CCloud::OnNavJumpHitApex( void )
{
...
//			jump state.
//---------------------------------------------------------
void CCloud::OnChangeActivity( Activity NewActivity )
{
	if ( NewActivity == ACT_FASTZOMBIE_FRENZY )
	{
		// Scream!!!!
		EmitSound( "NPC_Cloud.Frenzy" );
...
// 
//=========================================================
int CCloud::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	if ( m_fJustJumped )
	{
...
//=========================================================
void CCloud::BeginNavJump( void )
{
	m_fIsNavJumping = true;
	m_fHitApex = false;

	//ENVELOPE_CONTROLLER.SoundPlayEnvelope( m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envCloudVolumeJump3, ARRAYSIZE(envCloudVolumeJump3) );
}

//=========================================================
// 
//=========================================================
void CCloud::EndNavJump( void )
{
...
// 
//=========================================================
void CCloud::BeginAttackJump( void )
{
...
// 
//=========================================================
void CCloud::EndAttackJump( void )
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCloud::BuildScheduleTestBits( void )
{
...
// 
//=========================================================
void CCloud::OnStateChange( NPC_STATE OldState, NPC_STATE NewState )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCloud::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if( pEnt && pEnt->IsPlayer() )
	{
		...
		// Award the player a lot of experience.
		pPlayer->AddXP(10);
	}

	// Shut up my screaming sounds.
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "NPC_Cloud.NoSound" );
	StopLoopingSounds();

	// S:O - Just in case...
	StopSound( "NPC_Cloud.LeapAttack" );
	StopSound( "NPC_Cloud.FootstepRight" );
	StopSound( "NPC_Cloud.FootstepLeft" );
	StopSound( "NPC_Cloud.AttackHit" );
	StopSound( "NPC_Cloud.AttackMiss" );
	StopSound( "NPC_Cloud.LeapAttack" );
	StopSound( "NPC_Cloud.Attack" );
	StopSound( "NPC_Cloud.Idle" );
	StopSound( "NPC_Cloud.AlertFar" );
	StopSound( "NPC_Cloud.AlertNear" );
	StopSound( "NPC_Cloud.GallopLeft" );
	StopSound( "NPC_Cloud.GallopRight" );
	StopSound( "NPC_Cloud.Scream" );
	StopSound( "NPC_Cloud.RangeAttack" );
	StopSound( "NPC_Cloud.Frenzy" );
	StopSound( "NPC_Cloud.NoSound" );
	StopSound( "NPC_Cloud.Die" );
	StopSound( "NPC_Cloud.Gurgle" );
	StopSound( "NPC_Cloud.Moan1" );
	StopSound( "NPC_Cloud.Teleported" );
...
//-----------------------------------------------------------------------------
bool CCloud::CreateBehaviors( void )
{
...
//-----------------------------------------------------------------------------
void CCloud::InputAttachToVehicle( inputdata_t &inputdata )
{
	// Interrupt us
...
//-----------------------------------------------------------------------------
void CCloud::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
...
//-----------------------------------------------------------------------------
void CCloud::VehicleLeapAttack( void )
{
	CBaseEntity *pEnemy = GetEnemy();
	if ( pEnemy == NULL )
		return;
	...
	SetTouch( &CCloud::VehicleLeapAttackTouch );
}
...
//-----------------------------------------------------------------------------
bool CCloud::CanEnterVehicle( CPropJeepEpisodic *pVehicle )
{
	if ( pVehicle == NULL )
		return false;
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CCloud::VehicleLeapAttackTouch( CBaseEntity *pOther )
{
...
//-----------------------------------------------------------------------------
void CCloud::UpdateEfficiency( bool bInPVS )
{ 
...
AI_BEGIN_CUSTOM_NPC( npc_cloud, CCloud )
