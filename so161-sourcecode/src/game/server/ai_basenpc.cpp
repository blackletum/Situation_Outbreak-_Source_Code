...
#include "ilagcompensationmanager.h" // AI Patch Addition.
...
int CAI_Manager::AddAI( CAI_BaseNPC *pAI ) // AI Patch Addition.
 {
 	m_AIs.AddToTail( pAI );
	return NumAIs()-1; // return the index it was added to // AI Patch Addition.
 }
...
void CAI_BaseNPC::Event_Killed( const CTakeDamageInfo &info )
{
	...
	// S:O - Work out what killed the NPC
	...
	// S:O - New "npc_death" event implementation
	CBaseEntity * attacker = info.GetAttacker();
	IGameEvent * event = gameeventmanager->CreateEvent( "npc_death" );
	if ( event )
	{
		event->SetString( "victim", GetClassname() );
	...
	// S:O - Stop the annoying looping sounds already!
	EmitSound("AI_BaseNPC.NoSound");
}
...
/*#ifdef HL2_EPISODIC // AI Patch Addition.
	if ( AI_IsSinglePlayer() ) // AI Patch Addition.
	{
		CBasePlayer *pPlayer = AI_GetSinglePlayer(); // AI Patch Addition.
		if ( pPlayer->IRelationType( this ) != D_LI ) // AI Patch Addition.
		{ // AI Patch Addition.
			CNPC_Alyx *alyx = CNPC_Alyx::GetAlyx(); // AI Patch Addition.

			if ( alyx ) // AI Patch Addition.
			{ // AI Patch Addition.
				alyx->EnemyIgnited( this ); // AI Patch Addition.
			} // AI Patch Addition.
		} // AI Patch Addition.
	} // AI Patch Addition.
...
int CAI_BaseNPC::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( m_flLastDamageTime != gpGlobals->curtime )
	{
		...
		else
		{
			...
			if( pAttacker && pAttacker->IsAlive() && UTIL_GetNearestPlayer(GetAbsOrigin()) ) // AI Patch Addition.
			{
	...
	// S:O - New "npc_hurt" event implementation
	CBaseEntity * attacker = info.GetAttacker();
	...
	if ( event )
	{
		...
		if ( attacker->IsPlayer() )
		{
			CBasePlayer *pPlayer = ToBasePlayer( attacker );
			event->SetInt("attacker", pPlayer->GetUserID() );
			event->SetString("attackernpc", 0 );	// 0 when attacker is not a NPC
		}
		// Have we been hurt by an NPC?
		else if ( attacker->IsNPC() )
		{
...
void CAI_BaseNPC::UpdateSleepState( bool bInPVS )
{
	if ( GetSleepState() > AISS_AWAKE )
	{
		CBasePlayer *pLocalPlayer = UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
		if ( !pLocalPlayer )
		{
...
void CAI_BaseNPC::NPCThink( void )
{
	...
	// S:O - Rallypoint
	if( m_bShouldSearchForRallyPoint )
		FindRallyPoint();
}

// S:O - Rallypoint
void CAI_BaseNPC::SetNPCSpawnID( int ID )
{
	m_iOwner = ID;
}

// S:O - Rallypoint
int CAI_BaseNPC::GetNPCSpawnID()
{
	return m_iOwner;
}

// S:O - Rallypoint
void CAI_BaseNPC::FindRallyPoint()
{
	CBaseEntity *pIterated = NULL;
	CNPCRallyPoint *pNPCRallyPoint = NULL;
	
	while ( (pIterated = gEntList.FindEntityByClassname( pIterated, "so_spawner_rallypoint")) != NULL )
	{
		pNPCRallyPoint = dynamic_cast<CNPCRallyPoint*>( pIterated );

		if ( pNPCRallyPoint && pNPCRallyPoint->IsActive() && (GetNPCSpawnID() == pNPCRallyPoint->GetSpawnParent()) )
		{
			Vector forward;
			Vector vecGoal = pNPCRallyPoint->GetCoordinates();
			ConqCommanded( vecGoal, forward, true, this );
			m_bShouldSearchForRallyPoint = false;
		}
	}
...
void CAI_BaseNPC::RunAI( void )
{
	...
	if( ai_debug_loners.GetBool() && !IsInSquad() ) // AI Patch Addition.
	{
		Vector right;
		Vector vecPoint;
		...
		UTIL_GetNearestPlayer(GetAbsOrigin())->GetVectors( NULL, &right, NULL ); // AI Patch Addition.
...
void CAI_BaseNPC::StartNPC( void )
{
	...
	// S:O - Rallypoint
	m_bShouldSearchForRallyPoint = true;
}
...
void CAI_BaseNPC::DrawDebugGeometryOverlays(void)
{
	...
	if (m_debugOverlays & OVERLAY_NPC_KILL_BIT) 
	{
		...
		info.SetInflictor( (CBaseEntity *)this ); // AI Patch Addition.
...
CBaseEntity *CAI_BaseNPC::FindNamedEntity( const char *name, IEntityFindFilter *pFilter )
{
	...
	{
		return UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
	}
...
void CAI_BaseNPC::Precache( void )
{
	...
	// S:O - Stop the annoying looping sounds already! (precache)
	PrecacheScriptSound( "AI_BaseNPC.NoSound" );
    ...
    m_bDeferredNavigation( false )
{
	...
	SetAIIndex( g_AI_Manager.AddAI( this ) ); // AI Patch Addition.
	lagcompensation->RemoveNpcData( GetAIIndex() ); // make sure we're not inheriting anyone else's data // AI Patch Addition.
...
CAI_BaseNPC::~CAI_BaseNPC(void)
{
	...
	lagcompensation->RemoveNpcData( GetAIIndex() ); // AI Patch Addition.
	...
	if ( pOldCine && pOldCine->m_iszPlay != NULL_STRING && pOldCine->PlayedSequence() )
	{
		if ( !pOldCine->HasSpawnFlags(SF_SCRIPT_DONT_TELEPORT_AT_END) )
		{
			...
			else
			{
				...
				int drop = UTIL_DropToFloor( this, MASK_NPCSOLID, UTIL_GetNearestVisiblePlayer(this) ); // AI Patch Addition.
...
{
	CBasePlayer *pPlayer = UTIL_GetNearestPlayer(pNPC->GetAbsOrigin()); // AI Patch Addition.
	QAngle fan;
...
//		if ( !AI_IsSinglePlayer() ) // AI Patch Addition.
//			return false; // AI Patch Addition.
...
bool CAI_BaseNPC::FindNearestValidGoalPos( const Vector &vTestPoint, Vector *pResult )
{
	...
	if ( vCandidate != vec3_invalid )
	{
		AI_Waypoint_t *pPathToPoint = GetPathfinder()->BuildRoute( GetAbsOrigin(), vCandidate, UTIL_GetNearestPlayer(GetAbsOrigin()), 5*12, NAV_NONE, true ); // AI Patch Addition.
		if ( pPathToPoint )
		{
