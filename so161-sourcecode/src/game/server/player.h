...
public:
	...
	// S:O - Buyzone function declarations
	// Jordan @ June 28
	virtual void			EnableBuyZone(void){ buyzone = true;}
	virtual void			DisableBuyZone(void){ buyzone = false;}
	virtual bool			Buyzone(void){ return buyzone;}
	void					SwitchToMaiWeapon( void );	// the automatic weapon switch function for after purchasing a weapon

	// S:O - Buymenu Functionality
	void PurchaseWeapon( const char *szWeaponBuymenuName );
...
public:
	...
	// S:O - LUA
	int		GetScore() const { return m_iFrags;}
	void	IncrementScore(int score) { m_iFrags = m_iFrags + score;}
...
public:
	...
	// S:O - Misc.
	// Jordan @ Feb 10
	CNetworkVar( int,	m_iFOV );			// field of view - MOVED FOR ACCESS!
	CNetworkVar(int,	m_iMoney);
	CNetworkVar(int,	version);

	// S:O - Misc.
	bool					buyzone;
	int						m_iMenunumber;
	int						m_iLateJoiner;
	//int					m_iSpawnMusic;	// TODO: This looks old. Are we allowed to remove it?	// the answer is yes

	// S:O - Checks to see if the player is a certain type of zombie for class-based stuff
	bool IsCharple( void );
	bool IsCorpse( void );
	bool IsStalker( void );
	bool IsGhost( void );
...
public:
	...
	// S:O - Minimum amount of time before we're allowed to use the shout menu again (prevents spam).
	CountdownTimer m_AllowedToShoutTimer;
