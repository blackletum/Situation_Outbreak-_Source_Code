...
{
// 	if ( !AI_GetSinglePlayer() || AI_GetSinglePlayer()->IsDead() ) // AI Patch Addition.
...
{
	CBasePlayer *pPlayer = UTIL_GetNearestVisiblePlayer(GetOuter()); // AI Patch Addition.
	if ( !pPlayer ) // AI Patch Addition.
		pPlayer = UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
	SetTarget( pPlayer ); // AI Patch Addition.
...
bool CAI_LeadBehavior::PlayerIsAheadOfMe( bool bForce )
{
	...
	if ( GetClosestPointOnRoute( UTIL_GetNearestPlayer(GetAbsOrigin())->GetAbsOrigin(), &vecClosestPoint ) ) // AI Patch Addition.
	{
...
void CAI_LeadBehavior::GatherConditions( void )
{
	...
	if ( HasGoal() )
	{
		...
		CBaseEntity *pFollower = UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
...
int CAI_LeadBehavior::SelectSchedule()
{
	if ( HasGoal() )
	{
		...
		{
			CBasePlayer *pFollower = UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
...
void CAI_LeadBehavior::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		{
			m_MoveMonitor.SetMark( UTIL_GetNearestPlayer(GetAbsOrigin()), 24 ); // AI Patch Addition.
