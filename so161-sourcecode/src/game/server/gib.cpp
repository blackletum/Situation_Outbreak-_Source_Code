...
//  DEFINE_FIELD( m_bForceRemove, FIELD_BOOLEAN ),
	...
	DEFINE_THINKFUNC( MakeGibStatic ),
								...
								float			flLifetime)
{
	for (int i=0;i<nNumGibs;i++)
	{
		...
		pGib->SetThink( &CGib::MakeGibStatic );
		
		// S:O - Less laggy gibs (disable shadows and motion)
		pGib->SetEffects( EF_NOSHADOW );
		pGib->AddSolidFlags( FSOLID_NOT_SOLID );

		// S:O - BLOOD!!!
		UTIL_BloodDrips( pGib->GetAbsOrigin(), vec3_origin, BLOOD_COLOR_RED, 500 );
...
// S:O - Makes gibs static once they have stopped moving
// This results in even less lag caused by gibs since they won't interact physically with anything once frozen
void CGib::MakeGibStatic( void )
{
	// Check to see if the gib has stopped moving
	if ( GetAbsVelocity() == vec3_origin )
	{
		// If it has, freeze us right away and disappear
	...
	{
		// Check again in another tenth of a second or so
