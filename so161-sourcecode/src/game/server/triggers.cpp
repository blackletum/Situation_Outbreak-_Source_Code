...
void CChangeLevel::ChangeLevelNow( CBaseEntity *pActivator )
{
	...
	CBaseEntity *pPlayer = (pActivator && pActivator->IsPlayer()) ? pActivator : UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
...
private:
	...
	int	  m_state;
	Vector m_vecMoveDir;
	Vector m_vecLastPos;	// S:O - Fix for point_viewcontrol
...
BEGIN_DATADESC( CTriggerCamera )
	...
	DEFINE_FIELD( m_vecLastPos, FIELD_VECTOR),	// S:O - Fix for point_viewcontrol
...
void CTriggerCamera::Enable( void )
{
	...
	{
		m_hPlayer = UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
	}
	...
	m_vecLastPos = GetAbsOrigin();	// S:O - Fix for point_viewcontrol
	Move();
...
static void PlayCDTrack( int iTrack )
{
	...
	//Assert(gpGlobals->maxClients == 1); // AI Patch Addition.
...
//	>> S:O - TRIGGER_HOLDOUT
// ##################################################################################
class CHoldoutTrigger : public CBaseVPhysicsTrigger
{
DECLARE_CLASS( CHoldoutTrigger , CBaseVPhysicsTrigger );
...
private:
	...
	CountdownTimer m_TimeInZone;
	CUtlVector<CBaseEntity *> m_ZombiesInHoldoutAreaList;
};

BEGIN_DATADESC( CHoldoutTrigger )
END_DATADESC()

LINK_ENTITY_TO_CLASS( trigger_holdout, CHoldoutTrigger );

void CHoldoutTrigger::Spawn()
{
	BaseClass::Spawn();

	PrecacheScriptSound("HoldoutAlert");

	m_bHoldoutLost = false;
	m_bStartedTimer = false;

	SetNextThink( gpGlobals->curtime );	// start thinking
}

void CHoldoutTrigger::StartTimer()
{
	m_TimeInZone.Start( 5.0f );		// we'll try 5 seconds and see if that works well
	m_bStartedTimer = true;		// indicates that we've started the timer
}

void CHoldoutTrigger::StopTimer()
{
	// Doesn't actually stop the timer, just pretends it isn't running (we'll see if this actually works or not)
	//m_TimeInZone.Stop();		// stop the timer
	//m_TimeInZone.Invalidate();	// try invalidation?
	m_TimeInZone.Start( 0.0f );		// zero seconds? (trying everything here, not a perfect test but this seems to work - for now)
	m_bStartedTimer = false;		// indicates that we've stopped the timer
}

void CHoldoutTrigger::Think()
{
	// We need to find out if the zombies in our list are still alive or not.
	// Doing this is relatively simple! We check our zombie list to see if they are in there.
	// If they aren't, they must be dead, so we can remove them from our holdout area list.
	for ( int i = 0; i < m_ZombiesInHoldoutAreaList.Count(); i++ )
	{
		if ( gEntList.m_ZombieList.Find( m_ZombiesInHoldoutAreaList[i] ) == -1 )
			m_ZombiesInHoldoutAreaList.FindAndRemove( m_ZombiesInHoldoutAreaList[i] );
	}

	if ( m_bStartedTimer && m_TimeInZone.IsElapsed() )
	{
		if ( m_ZombiesInHoldoutAreaList.Count() > 0 )	// must be at least one zombie still in zone to declare failure
		{
			...
			UTIL_HudMessageAll( tTextParam, "MISSION FAILED!\nThe zombie horde breached the holdout zone!\n" );
			...
			for ( int i = 1; i <= gpGlobals->maxClients; i++ )
			{
				...
				if ( pPlayer2 )
				{
					engine->ClientCommand( pPlayer2->edict(), "play events/so-zombieswin.mp3" );
				}
			}
			...
			if ( pRules )
			{
				pRules->SetRoundState( RoundEnding, 10.0f );
				pRules->RoundEndOverlayFired = true;
			}

			m_bHoldoutLost = true;	// the holdout zone has been lost! oh no!
		}
		else	// Only declare the holdout zone cleared if there aren't any zombies to be found in it
		{
			hudtextparms_s tTextParam;
			tTextParam.x			= -1;
			tTextParam.y			= 0.60;
			...
			tTextParam.fxTime		= 1.0;
			tTextParam.channel		= 1;
			UTIL_HudMessageAll( tTextParam, "Holdout zone sterilized - for now...\nPrevent any future intrusions by defending the holdout zone." );
		}

		// Intruder has left holdout area or has been killed or something
		if ( m_bStartedTimer )
			StopTimer();	// stop our timer if it is still running
	}

	SetNextThink( gpGlobals->curtime + 1 );	// think again in one second
}

void CHoldoutTrigger::StartTouch( CBaseEntity *pOther )
{
	if( pOther )
	{
		CHL2MPRules *pRules = HL2MPRules();
		if( !m_bHoldoutLost && pRules && pRules->GetRoundState() != RoundEnding )
		{
			CBasePlayer *pPlayer = ToBasePlayer( pOther );
			...
			const char* classnamew = pOther->GetClassname();
			if ( (FStrEq(classnamew, "npc_fastzombie") || FStrEq(classnamew, "npc_poisonzombie") || FStrEq(classnamew, "npc_zombie") || FStrEq(classnamew, "npc_sploderzombie")) && !pRules->RoundEndOverlayFired )
			{
				// Only declare the alert if there wasn't at least one zombie in the zone before
				if ( m_ZombiesInHoldoutAreaList.Count() == 0 )
				{
					// Intruder has entered the holdout area!
					// Give the survivors a chance to get it out before declaring failure
					StartTimer();

					EmitSound("HoldoutAlert");
					...
					tTextParam.fadeinTime	= 1.0;
					tTextParam.fadeoutTime	= 1.0;
					tTextParam.holdTime		= 8.0;
					tTextParam.fxTime		= 1.0;
					tTextParam.channel		= 1;
					UTIL_HudMessageAll( tTextParam, "INTRUDER ALERT!\nThe holdout zone has been compromised!\nClear the holdout zone before it's too late..." );
				}

				if ( m_ZombiesInHoldoutAreaList.Find( pOther ) == -1 )
					m_ZombiesInHoldoutAreaList.AddToTail( pOther );	// make note of this zombie (as long as we haven't already, of course)
			}
		}
	}
}

void CHoldoutTrigger::EndTouch( CBaseEntity *pOther )
{
	if( pOther )
	{
		CHL2MPRules *pRules = HL2MPRules();
		if( !m_bHoldoutLost && pRules && pRules->GetRoundState() != RoundEnding )
		{
			...
			const char* classnamew = pOther->GetClassname();
			if ( (FStrEq(classnamew, "npc_fastzombie") || FStrEq(classnamew, "npc_poisonzombie") || FStrEq(classnamew, "npc_zombie") || FStrEq(classnamew, "npc_sploderzombie")) && !pRules->RoundEndOverlayFired )
				m_ZombiesInHoldoutAreaList.FindAndRemove( pOther );	// make note of this zombie (left zone)
		}
	}
}
...
//	>> S:O - TRIGGER_ESCAPE
// ##################################################################################
class CEscapeTrigger : public CBaseVPhysicsTrigger
{
DECLARE_CLASS( CEscapeTrigger , CBaseVPhysicsTrigger );
...
BEGIN_DATADESC( CEscapeTrigger )
END_DATADESC()

LINK_ENTITY_TO_CLASS( trigger_escape, CEscapeTrigger );

void CEscapeTrigger::Spawn( void )
{
	BaseClass::Spawn();
	m_bEscapedYay = false;
}

void CEscapeTrigger::StartTouch( CBaseEntity *pOther )
{
	if( pOther )
	{
		CHL2MPRules *pRules = HL2MPRules();
		if( !m_bEscapedYay && pRules && pRules->GetRoundState() != RoundEnding )
		{
			CBasePlayer *pPlayer = ToBasePlayer( pOther );

			if( pPlayer && pPlayer->GetTeamNumber() == 3 && pPlayer->IsAlive() && !pRules->RoundEndOverlayFired )
			{
				// If this player hasn't been added to our list yet, add them now.
				if ( gEntList.m_EscapedList.Find( pPlayer ) == -1 )
				{
					gEntList.m_EscapedList.AddToTail( pPlayer );
				}

				// If we've accounted for all of the Survivors and all have reached the escape point, the Survivors win!
				if ( gEntList.m_PlayerList.Count() == gEntList.m_EscapedList.Count() )
				{
					hudtextparms_s tTextParam;
					tTextParam.x			= -1;
					tTextParam.y			= -1;
					...
					tTextParam.fxTime		= 1.0;
					tTextParam.channel		= 1;
					UTIL_HudMessageAll( tTextParam, "The Survivors escaped from the zombie horde!\n" );
					...
					for ( int i = 1; i <= gpGlobals->maxClients; i++ )
					{
						...
						if ( pPlayer2 )
						{
							engine->ClientCommand( pPlayer2->edict(), "play events/so-survivorswin.mp3" );
						}
					}
					...
					// Purge our list of escaped Survivors here, just in case.
					// This is also done later in hl2mp_gamerules.cpp, but whatever.
...
void CEscapeTrigger::EndTouch( CBaseEntity *pOther )
{
	if( pOther )
	{
		CHL2MPRules *pRules = HL2MPRules();
		if( !m_bEscapedYay && pRules && pRules->GetRoundState() != RoundEnding )
		{
			...
			if( pPlayer )
			{
				// If this player hasn't been removed from our list yet, remove them now.
				gEntList.m_EscapedList.FindAndRemove( pPlayer );
			}
		}
	}
}
...
//	>> S:O - ZMS_BUYZONE
// ##################################################################################
class CZMSBuyZone : public CTriggerMultiple
{
	DECLARE_CLASS( CTriggerOnce, CTriggerMultiple );
...
LINK_ENTITY_TO_CLASS( zms_buyzone, CZMSBuyZone );
LINK_ENTITY_TO_CLASS( so_buyzone, CZMSBuyZone );

void CZMSBuyZone::StartTouch( CBaseEntity *pOther )
{
	if( pOther && pOther->IsPlayer() )
	{
		CBasePlayer *pPlayer = dynamic_cast<CBasePlayer *>( pOther );
		if( pPlayer && !pPlayer->Buyzone() && (pPlayer->GetTeamNumber() == 3 || pPlayer->GetTeamNumber() == 4) )
		{
			pPlayer->EnableBuyZone();
			UTIL_HudHintText( pPlayer, "You've found a buyzone! Press B to purchase weapons and ammo." );
		}
	}
}

void CZMSBuyZone::EndTouch( CBaseEntity *pOther )
{
	if( pOther && pOther->IsPlayer() )
	{
		CBasePlayer *pPlayer = dynamic_cast<CBasePlayer *>( pOther );
		if( pPlayer && pPlayer->Buyzone() )
		{
			pPlayer->DisableBuyZone();
		}
	}
}