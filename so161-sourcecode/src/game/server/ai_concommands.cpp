...
// S:O - Control NPC Movement
//---------------------------------------------------------------------------------
// S:O - Commands all zombie NPCs to follow the player who executes this commmand.
//---------------------------------------------------------------------------------
CON_COMMAND ( zombie_regroup, "Commands all zombie NPCs to follow the player who executes this command" )
{
	CBasePlayer *pPlayer = ToBasePlayer( UTIL_GetCommandClient() );

	if ( pPlayer && pPlayer->IsAlive() && pPlayer->GetTeamNumber() == 2 )
	{
		...
		for ( int i = 0; i < gEntList.m_ZombieList.Count(); i++)
		{
			...
			if ( pSelector )
			{
				...
				CAI_BaseNPC::ConqCommanded( vecGoal, forward, true, pSelector );
			}
		}
	}
}
...
//------------------------------------------------------------------------------------------------------------
// S:O - Commands all zombie NPCs to go to the coordinates specified by the player who executes this command.
//------------------------------------------------------------------------------------------------------------
CON_COMMAND ( zombie_goto, "Commands all zombie NPCs to go to the coordinates specified by the player who executes this command" )
{
	CBasePlayer *pPlayer = ToBasePlayer( UTIL_GetCommandClient() );
	...
	if ( pPlayer && pPlayer->IsAlive() && pPlayer->GetTeamNumber() == 2 )
	{
		...
		tracetarget.x = atof( args[ 1 ] );
		tracetarget.y = atof( args[ 2 ] );
		tracetarget.z = atof( args[ 3 ] );
