...
CEntityFlame::~CEntityFlame( void )
{
	// S:O - Stop any possible looping fire sounds upon removal
...
void CEntityFlame::FlameThink( void )
{
	...
	if ( m_flLifetime < gpGlobals->curtime || m_hEntAttached == NULL || m_hEntAttached->GetTeamNumber() == 1 )	// S:O - Extinguish any fires when a player becomes a spectator
	{
		EmitSound( "General.StopBurning" );
		m_bPlayingSound = false;
