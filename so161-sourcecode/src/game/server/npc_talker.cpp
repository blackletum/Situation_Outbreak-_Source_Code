...
void CNPCSimpleTalker::RunTask( const Task_t *pTask )
{
	...
	case TASK_TALKER_LOOK_AT_CLIENT:
		...
		if ( pTask->iTask == TASK_TALKER_CLIENT_STARE ) // AI Patch Addition.
		{
			// Get edict for one player
			//CBasePlayer *pPlayer = UTIL_GetLocalPlayer(); // AI Patch Addition.
			//Assert( pPlayer ); // AI Patch Addition.
			CBasePlayer *pPlayer = UTIL_GetNearestVisiblePlayer(this); // AI Patch Addition.

			// fail out if the player looks away or moves away.
			if ( !pPlayer || ( pPlayer->GetAbsOrigin() - GetAbsOrigin() ).Length2D() > TALKER_STARE_DIST ) // AI Patch Addition.
			{
				// player moved away.
...
int CNPCSimpleTalker::SelectNonCombatSpeechSchedule()
{
	...
	if ( GetExpresser()->CanSpeak() && HasCondition ( COND_SEE_PLAYER ) && random->RandomInt( 0, 6 ) == 0 ) // AI Patch Addition.
	{ // AI Patch Addition.
CBasePlayer *pPlayer = UTIL_GetNearestVisiblePlayer(this); // AI Patch Addition.
