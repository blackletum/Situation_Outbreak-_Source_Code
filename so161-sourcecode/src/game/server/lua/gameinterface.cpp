...
extern void RoundRestart();
extern ConVar zms_roundtime;
...
ConVar zms_roundtime( "zms_roundtime", "10.0", FCVAR_GAMEDLL, "Defines how long a round lasts");
ConVar *sv_maxreplay = NULL;
extern ConVar zms_gamemode;
...
bool CServerGameDLL::GameInit( void )
{
	...
	engine->ServerCommand( "exec zmsconfig.cfg\n" );
...
bool CServerGameDLL::LevelInit( const char *pMapName, char const *pMapEntities, char const *pOldLevel, char const *pLandmarkName, bool loadGame, bool background )
{
	...
	int iRound = zms_roundtime.GetInt();
	int result = (iRound * 60 );
	pRules->StartRoundtimer(result);
	pRules->SetRoundState( RoundStarted, (float)result );
	// clear any pending autosavedangerous
	m_fAutoSaveDangerousTime = 0.0f;
	m_fAutoSaveDangerousMinHealthToCommit = 0.0f;
	if (FStrEq(zms_gamemode.GetString(), "Holdout"))
	{
		CBaseEntity *pTarget = gEntList.FindEntityByClassname(NULL, "CHoldOutTrigger");
		if(pTarget)
		{
			CBaseEntity *pEntity = CreateEntityByName("CRallyPoint");
			if(pEntity)
			{
				pEntity->KeyValue("name", "holdout_rally");
				pEntity->SetAbsOrigin(pTarget->GetAbsOrigin());
			}
		}
	
	}
		DevMsg("Connecting to Angel...\n");

		CAngel *pAngel = CAngel::GetAngel();

		if( pAngel->GetStatusFlags() & A_OK )

		{
			DevMsg("Connected to Angel web services at address: %s. Session ID: %s. Expire Time: %s\n", 

				ANGEL_ADDRESS, pAngel->GetSessionID(), pAngel->GetExpireTime());
		}
		else if( pAngel->GetStatusFlags() & A_IN_ERROR )
		{
			DevWarning("Failed to connect to Angel web services, reason: %s\n", pAngel->GetLastError());

		}
		else
		{
			DevWarning("Unknown Angel status!\n");
		}
	return true;
}
