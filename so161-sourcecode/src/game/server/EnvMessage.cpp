...
void CMessage::InputShowMessage( inputdata_t &inputdata )
{
	...
	else
	{
		...
		{
			pPlayer = UTIL_GetLocalPlayer(); // just show it to the host, if there is one // AI Patch Addition.
		}
...
void CCredits::RollOutroCredits()
{
	...
	CRecipientFilter filter; // AI Patch Addition.
	filter.AddAllPlayers(); // AI Patch Addition.
	filter.MakeReliable(); // AI Patch Addition.
	UserMessageBegin( filter, "CreditsMsg" ); // AI Patch Addition.
		WRITE_BYTE( 3 );
	MessageEnd();
}
...
void CCredits::InputShowLogo( inputdata_t &inputdata )
{
	...
	// Modification. Set to how old patched AI SDK had code. // AI Patch Addition.
	//CSingleUserRecipientFilter user( pPlayer ); // AI Patch Addition.
	//user.MakeReliable(); // AI Patch Addition.

	if ( m_flLogoLength )
	{
		UserMessageBegin( filter, "LogoTimeMsg" ); // AI Patch Addition.
...
void CCredits::InputRollCredits( inputdata_t &inputdata )
{
	...
	UserMessageBegin( filter, "CreditsMsg" ); // AI Patch Addition.
WRITE_BYTE( 2 ); // Modification: Added from old patched AI SDK. // AI Patch Addition.
	MessageEnd();
}
