...
#include "ai_basenpc.h" // AI Patch Addition.
...
public:
	...
	void RemoveNpcData(int index) // clear specific NPC's history // AI Patch Addition.
	{ // AI Patch Addition.
		CUtlFixedLinkedList< LagRecord > *track = &m_EntityTrack[index]; // AI Patch Addition.
		track->Purge(); // AI Patch Addition.
	} // AI Patch Addition.


private:
	void			BacktrackPlayer( CBasePlayer *player, float flTargetTime );
	void			BacktrackEntity( CAI_BaseNPC *entity, float flTargetTime ); // AI Patch Addition.

	void ClearHistory()
	{
		for ( int i=0; i<MAX_PLAYERS; i++ )
			m_PlayerTrack[i].Purge();
			for ( int j=0; j<MAX_AIS; j++ ) // AI Patch Addition.
			m_EntityTrack[j].Purge(); // AI Patch Addition.
	}
void UpdateAIIndexes(); // AI Patch Addition.
	// keep a list of lag records for each player
	CUtlFixedLinkedList< LagRecord >	m_PlayerTrack[ MAX_PLAYERS ];
	CUtlFixedLinkedList< LagRecord >	m_EntityTrack[ MAX_AIS ]; // AI Patch Addition.

	// Scratchpad for determining what needs to be restored
	CBitVec<MAX_PLAYERS>	m_RestorePlayer;
	CBitVec<MAX_AIS>		m_RestoreEntity; // AI Patch Addition.
	...
	LagRecord				m_EntityRestoreData[ MAX_AIS ]; // AI Patch Addition.
	LagRecord				m_EntityChangeData[ MAX_AIS ]; // AI Patch Addition.

	CBasePlayer				*m_pCurrentPlayer;	// The player we are doing lag compensation for
	
	bool					m_bNeedsAIUpdate; // AI Patch Addition.
};
...
{
		// sort out any changes to the AI indexing // AI Patch Addition.
	if ( m_bNeedsAIUpdate ) // to be called once per frame... must happen BEFORE lag compensation - // AI Patch Addition.
	{// if that happens, that is. if not its called at the end of the frame // AI Patch Addition.
		m_bNeedsAIUpdate = false; // AI Patch Addition.
		UpdateAIIndexes(); // AI Patch Addition.
	} // AI Patch Addition.

	// Assume no players or entities need to be restored // AI Patch Addition.

	m_RestorePlayer.ClearAll();
	m_RestoreEntity.ClearAll(); // AI Patch Addition.
	...
	Q_memset( m_EntityRestoreData, 0, sizeof( m_EntityRestoreData ) ); // AI Patch Addition.
	Q_memset( m_EntityChangeData, 0, sizeof( m_EntityChangeData ) ); // AI Patch Addition.
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( !pPlayer || player == pPlayer ) // AI Patch Addition.
			continue;
	...
	// also iterate all monsters // AI Patch Addition.
	CAI_BaseNPC **ppAIs = g_AI_Manager.AccessAIs(); // AI Patch Addition.
	int nAIs = g_AI_Manager.NumAIs(); // AI Patch Addition.

	for ( int i = 0; i < nAIs; i++ ) // AI Patch Addition.
	{ // AI Patch Addition.
		CAI_BaseNPC *pNPC = ppAIs[i]; // AI Patch Addition.
		// Custom checks for if things should lag compensate // AI Patch Addition.
		if ( !pNPC || !player->WantsLagCompensationOnEntity( pNPC, cmd, pEntityTransmitBits ) ) // AI Patch Addition.
			continue; // AI Patch Addition.
		
		// Move NPC back in time // AI Patch Addition.
		BacktrackEntity( pNPC, TICKS_TO_TIME( targettick ) ); // AI Patch Addition.
	} // AI Patch Addition.
}
