...
void CGameText::Display( CBaseEntity *pActivator )
{
	...
	// also send to all if we haven't got a specific activator player to send to // AI Patch Addition.
	if ( MessageToAll() || !pActivator || !pActivator->IsPlayer() ) // AI Patch Addition.
	{
...
//----------------------------------------------------------------------------------------------
// S:O - so_hud_message
// An I/O controlled HUD message that appears on all players' screens when triggered to do so.
//----------------------------------------------------------------------------------------------

class CZMSGameText : public CGameText
{
public:
	DECLARE_CLASS(CZMSGameText, CGameText);
	DECLARE_DATADESC();

	void InputDisplayText( inputdata_t &inputdata );
};

LINK_ENTITY_TO_CLASS( zms_hud_message, CZMSGameText );
LINK_ENTITY_TO_CLASS( so_hud_message, CZMSGameText );

BEGIN_DATADESC( CZMSGameText )
	DEFINE_INPUTFUNC( FIELD_STRING, "DisplayText", InputDisplayText ),
END_DATADESC()

void CZMSGameText::InputDisplayText( inputdata_t &inputdata )
{
...
//----------------------------------------------------------------------------------------------
// S:O - so_gamemode
// Allows mappers to easily define what gamemode they are mapping for.
// NOTE: Legacy methods will always be supported, but this using this entity is MUCH easier.
//----------------------------------------------------------------------------------------------

class CZMSGamemode : public CRulePointEntity
{
public:
	DECLARE_CLASS( CZMSGamemode, CRulePointEntity );
	bool KeyValue( const char *szKeyName, const char *szValue );
	DECLARE_DATADESC();
...
LINK_ENTITY_TO_CLASS( zms_gamemode, CZMSGamemode );
LINK_ENTITY_TO_CLASS( so_gamemode, CZMSGamemode );

BEGIN_DATADESC( CZMSGamemode )
	DEFINE_KEYFIELD( m_iszGamemode, FIELD_STRING, "gamemode" ),
END_DATADESC()

bool CZMSGamemode::KeyValue( const char *szKeyName, const char *szValue )
{
	if ( FStrEq( szKeyName, "Gamemode" ) || FStrEq( szKeyName, "gamemode" ) )
	{
		if ( FStrEq( szValue, "Survival" ) || FStrEq( szValue, "survival" ) )
		{
			engine->ServerCommand("so_gamemode Survival\n");
		}
		else if ( FStrEq( szValue, "Infection" ) || FStrEq( szValue, "infection" ) )
		{
			engine->ServerCommand("so_gamemode Infection\n");
		}
		else if ( FStrEq( szValue, "Holdout" ) || FStrEq( szValue, "holdout" ) )
		{
			engine->ServerCommand("so_gamemode Holdout\n");
		}
		else if ( FStrEq( szValue, "Escape" ) || FStrEq( szValue, "escape" ) )
		{
			engine->ServerCommand("so_gamemode Escape\n");
		}
		else if ( FStrEq( szValue, "Objective" ) || FStrEq( szValue, "objective" ) )
		{
			engine->ServerCommand("so_gamemode Objective\n");
		}
		else if ( FStrEq( szValue, "Overlord" ) || FStrEq( szValue, "overlord" ) )
		{
			engine->ServerCommand("so_gamemode Overlord\n");
		}
		else if ( FStrEq( szValue, "Extermination" ) || FStrEq( szValue, "extermination" ) )
		{
			engine->ServerCommand("so_gamemode Extermination\n");
		}
		else
		{
			// Gamemode field missing or not defined! Silly mappers!
			// Don't do anything here except return so we can figure out what to do later.
			return BaseClass::KeyValue( szKeyName, szValue );
		}
	}
	else
