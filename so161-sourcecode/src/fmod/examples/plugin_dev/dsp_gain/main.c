...
FMOD_RESULT F_CALLBACK dspreset(FMOD_DSP_STATE *dsp)
{
    ...
    /*
        This callback isnt really needed for a gain filter.  It is used to reset a filter back to 
        it's default state. This is where buffers would be cleared so that the filter is ready for
        reuse.
    */
